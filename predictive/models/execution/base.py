...
class TimeTemperatureStep(BaseModel):
    ...
    step_order: int = Field(ge=1, description="Order in the sequence (1-indexed)")
...
class TimeTemperatureProfile(BaseModel):
    ...
    is_multi_step: bool = Field(
        default=False, description="Whether this is a multi-step profile"
    )
    steps: list[TimeTemperatureStep] = Field(
        min_length=1, description="Time-temperature steps (at least one required)"
    )
    total_duration_minutes: float = Field(
        gt=0, description="Total duration across all steps"
    )
...
class BaseExecutionPayload(BaseModel):
    ...
    engine_type: EngineType = Field(description="Which engine implementation to use")
...
class BaseModelResult(BaseModel):
    ...
    model_type: ModelType = Field(description="Type of model that was run")
    engine_type: EngineType = Field(description="Which engine produced this result")
...
class GrowthPrediction(BaseModel):
    ...
    step_order: int = Field(description="Which step this prediction is for")
    duration_minutes: float = Field(description="Duration of this step")
    temperature_celsius: float = Field(description="Temperature during this step")
...
class BaseExecutionResult(BaseModel):
    ...
    step_predictions: list[GrowthPrediction] = Field(
        default_factory=list, description="Predictions for each time-temperature step"
    )
    ...
    # Metadata
    engine_type: EngineType = Field(description="Which engine was used")
