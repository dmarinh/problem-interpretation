...
class ComBaseParameters(BaseModel):
    ...
    factor4_type: Factor4Type = Field(
        default=Factor4Type.NONE, description="Type of fourth factor if applicable"
    )
    factor4_value: float | None = Field(
        default=None, description="Value of fourth factor (units depend on type)"
    )
    ...
    def validate_factor4(self) -> "ComBaseParameters":
        ...
        if self.factor4_type != Factor4Type.NONE and self.factor4_value is None:
            raise ValueError(
                f"factor4_value required when factor4_type is {self.factor4_type}"
            )
        return self
...
class ComBaseModelSelection(BaseModel):
    ...
    organism: ComBaseOrganism = Field(description="Target organism")
...
class ComBaseExecutionPayload(BaseExecutionPayload):
    ...
    # Model selection
    model_selection: ComBaseModelSelection = Field(description="Which model to run")
...
class ComBaseModelResult(BaseModelResult):
    ...
    doubling_time_hours: float | None = Field(
        default=None, description="Doubling time in hours (for growth models only)"
    )
    y_max: float = Field(
    ...
    # Organism
    organism: ComBaseOrganism = Field(description="Organism modeled")

    # Parameters used (for traceability)
    temperature_used: float = Field(description="Temperature used in calculation")
    ph_used: float = Field(description="pH used in calculation")
    aw_used: float = Field(description="Water activity used in calculation")
    factor4_type_used: Factor4Type = Field(
        default=Factor4Type.NONE, description="Fourth factor type used"
    )
    factor4_value_used: float | None = Field(
        default=None, description="Fourth factor value used"
    )
...
class ComBaseExecutionResult(BaseExecutionResult):
    ...
    # Override engine_type with ComBase default
    engine_type: EngineType = Field(
        default=EngineType.COMBASE_LOCAL, description="Which engine was used"
    )

    # ComBase-specific model result
    model_result: ComBaseModelResult = Field(description="Raw model calculation result")
