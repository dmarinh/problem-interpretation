...
- CONFIRMED: `organism` has NEVER had a code-level default — `standardization_service._get_organism()` (line 263-265) just returns `grounded.get("organism")`, no fallback, and `settings.py` has no organism-related setting at all. Reviewed and verified this via grep; it's not new in this change.
...
- `missing_required` in `StandardizationResult` is a short, closed list of producers (grep for `missing_required.append` — 4 call sites as of A0.5b: ungrounded organism, non-executable organism, missing single-step temperature [defensive/near-unreachable], missing multi-step step duration). `_missing_key_to_audit_key()`'s string-prefix matching (`.startswith("organism (")`) is safe specifically because this list is closed and reviewable in one grep — re-verify the producer count hasn't grown before trusting a prefix-match audit-key mapping in a future review.
