...
class ExtractedTemperature(BaseModel):
    ...
    value_celsius: float | None = Field(
        default=None, description="Explicit temperature value in Celsius if mentioned"
    )
    description: str | None = Field(
        default=None,
        description="Descriptive term used (e.g., 'room temperature', 'refrigerated', 'warm')",
    )
    is_range: bool = Field(default=False, description="Whether a range was specified")
    range_min_celsius: float | None = Field(
        default=None, description="Minimum of range if specified"
    )
    range_max_celsius: float | None = Field(
        default=None, description="Maximum of range if specified"
    )
...
class ExtractedDuration(BaseModel):
    ...
    is_ambiguous: bool = Field(
        default=False, description="Whether the duration is vague/ambiguous"
    )
...
class ExtractedTimeTemperatureStep(BaseModel):
    ...
    temperature: ExtractedTemperature = Field(
        default_factory=ExtractedTemperature, description="Temperature during this step"
    )
    duration: ExtractedDuration = Field(
        default_factory=ExtractedDuration, description="Duration of this step"
    )
    sequence_order: int | None = Field(
        default=None, description="Order in the sequence (1, 2, 3...)"
    )
...
class ExtractedFoodProperties(BaseModel):
    ...
    aw_value: float | None = Field(
        default=None, description="Single water activity value if explicitly stated"
    )
    aw_min: float | None = Field(
        default=None, description="Minimum water activity if a range is given"
    )
    aw_max: float | None = Field(
        default=None, description="Maximum water activity if a range is given"
    )
    ...
    def has_ph(self) -> bool:
        return (
            ...
            or self.ph_min is not None
            or self.ph_max is not None
        )

    @property
    def has_aw(self) -> bool:
        return (
            self.aw_value is not None
            or self.aw_min is not None
            or self.aw_max is not None
        )
...
class ExtractedEnvironmentalConditions(BaseModel):
    ...
    ph_description: str | None = Field(
        default=None, description="pH description (e.g., 'acidic', 'neutral')"
    )
    ...
    salt_percent: float | None = Field(
        default=None, description="Salt/NaCl percentage if mentioned"
    )
    salt_description: str | None = Field(
        default=None, description="Salt description (e.g., 'salty', 'brined')"
    )
    co2_percent: float | None = Field(
        default=None, description="CO2 percentage if mentioned (modified atmosphere)"
    )
    nitrite_ppm: float | None = Field(
        default=None, description="Nitrite concentration in ppm if mentioned"
    )
    lactic_acid_ppm: float | None = Field(
        default=None, description="Lactic acid concentration in ppm if mentioned"
    )
    acetic_acid_ppm: float | None = Field(
        default=None, description="Acetic acid concentration in ppm if mentioned"
    )
...
class ExtractedScenario(BaseModel):
    ...
    time_temperature_steps: list[ExtractedTimeTemperatureStep] = Field(
        default_factory=list, description="Multiple steps for complex scenarios"
    )
    ...
    additional_context: str | None = Field(
        default=None, description="Any other relevant context mentioned"
    )

    # Scenario type inference (NEW)
    is_cooking_scenario: bool = Field(
        default=False, description="Whether this is a cooking/heating scenario"
    )
    is_storage_scenario: bool = Field(
        default=False, description="Whether this is a storage/holding scenario"
    )
    ...
    implied_model_type: ModelType | None = Field(
        default=None, description="Model type implied by scenario context"
    )
...
class ExtractedIntent(BaseModel):
    ...
    reasoning: str | None = Field(
        default=None, description="Brief explanation of why this intent was assigned"
    )
...
class ClarificationQuestion(BaseModel):
    ...
    options: list[str] | None = Field(
        default=None, description="Suggested options for the user (if applicable)"
    )
    default_if_skipped: str | None = Field(
        default=None, description="What value will be used if user doesn't respond"
    )
...
class ExtractedClarificationResponse(BaseModel):
    ...
    understood_value: str | None = Field(
        default=None, description="The value extracted from user's clarification"
    )
    ...
    wants_to_skip: bool = Field(
        default=False, description="Whether user wants to skip and use default"
    )
    additional_info: str | None = Field(
        default=None, description="Any additional information provided"
    )
