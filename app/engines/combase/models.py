...
class ComBaseModel(BaseModel):
    ...
    # Identification
    model_id: int = Field(
        description="ComBase ModelID (1=Growth, 2=Thermal, 3=Non-thermal)"
    )
    ...
    factor4_type: Factor4Type = Field(
        default=Factor4Type.NONE, description="Fourth factor type"
    )
        ...
        with open(csv_path, encoding="utf-8-sig") as f:
            ...
            for row in reader:
                ...
                try:
                    ...
                    print(
                        f"Warning: Failed to parse row: {row.get('Org', 'unknown')} - {e}"
                    )
                    continue
