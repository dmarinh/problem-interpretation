...
- **ComBaseEngine** (`predictive/engines/combase/engine.py`): Iterates over `TimeTemperatureProfile.steps`, accumulating log growth per step via `GrowthPrediction` objects into `step_predictions[]`. Lives in the top-level `predictive/` library (relocated from `app/engines/` — see specs/lessons.md), a peer of `app/` that imports nothing from it; `app/` consumes it as a library.
