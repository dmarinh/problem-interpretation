...
**Insights after testing**
- What we learned: The system failed on duration as expected — the query describes a cooling arc ("60°C at end of cooking → refrigeration temperature") without providing a timeline, and duration is now a required field. The more informative finding was in the temperature extraction. The semantic parser extracted 60°C (the only temperature in the query) and treated it as the static modelling temperature, then clamped it to the growth model's upper boundary of 52°C. This is a semantic misread: 60°C in this query is the starting condition of a dynamic cooling scenario, not an operating temperature. At 60°C, C. perfringens is above its growth range and not growing at all. The biologically relevant portion of the scenario is the cooling window from ~52°C down to ~15°C, and the critical parameter is the duration spent there — which the query does not supply. The pH retrieval also surfaced a pattern worth noting: the query "cooked beef stew pH acidity" returned only cooked vegetables as candidates (chayote, okra, Swiss chard), with no meat-relevant documents in range. The embedder is responding to the word "cooked" rather than the food category, and "beef stew" as a composite food term provides no usable signal. The aw retrieval almost found "fresh meat (meat): 0.99–1.0" at 0.5843 embedding score but was blocked by the composite blocklist, which is working as designed.
- What we fixed: Nothing this round. The structured error on duration is correct and principled. The composite blocklist preventing uncritical inheritance of fresh-meat properties for a stew is also correct. The temperature semantic misread and the pH retrieval gap for composite foods are both real issues, but they belong to existing roadmap tracks: dynamic/multi-step scenario parsing and the composite-food strategy respectively.
- What this taught us: A3 exposes a class of query the system has no model for yet — cooling scenarios described as a trajectory ("starts at X, needs to reach Y") rather than as a static temperature. The parser has no way to distinguish "the stew is at 60°C" (starting condition) from "the stew is held at 60°C" (static operating temperature). Both produce the same extraction but mean completely different things physically. The dynamic scenario parsing work that was flagged in B1 (truck cold-start ramp) and in the multi-step Q17 entry in the test journal is the same architectural gap surfaced here from a different angle: the system needs a way to recognise trajectory-type temperature descriptions and respond with either a structured error naming the trajectory as the missing piece, or a multi-step decomposition with user-supplied waypoints. The 1-log USDA criterion in the query also marks the outer boundary of PTM's current scope — producing a log prediction is within reach once duration is supplied, but adjudicating whether that prediction meets a regulatory standard is the Result Interpretation Module's job.
...
What we learned: The query asks for the bacterial load on a ready-to-eat salad after cross-contamination from raw chicken via a cutting board. This is structurally a transfer-plus-growth problem: it requires an initial pathogen concentration on the chicken, a transfer rate from board to salad (literature values span 0.01–10%), and growth during the 2-hour holding period. PTM modelled only the third component and produced a log_increase of 0.87 (about 7×), with no warning that the cross-contamination step was not addressed. The user, reading this output without knowing the system internals, would likely interpret 0.87 log as "the bacterial load on the salad" — which is not what was computed. Beyond the scope mismatch, three smaller findings emerged. The temperature was inferred as 25°C through a rule_match against the canonical pattern "room temperature", even though the query does not contain that phrase — the input was "domestic kitchen", and the LLM extractor (or an alias map) silently normalised it before the rule fired. The audit records the matched pattern but not the original phrase that produced the match. The pH retrieval ran against "ready-to-eat salad pH acidity" and surfaced acidified condiments and acidified canned vegetables as runners-up, none of which are right for a fresh salad — the embedder is again responding to "pH acidity" rather than the food category, the same failure pattern as A3's "cooked beef stew" returning cooked vegetables. The aw retrieval came tantalisingly close: fresh vegetables (water activity 0.97–1.0) scored 0.6131, just below the 0.62 gate, and was skipped — the conservative default of 0.99 happens to fall inside the missed document's range, but the path to the answer is "default fired", not "evidence retrieved".
What we fixed: Nothing this round. Each finding belongs to existing roadmap tracks. The cross-contamination scope mismatch is a structural limitation — PTM is a predictive-microbiology parameter standardiser, not a transfer model, and should explicitly recognise transfer queries and either warn or refuse. A scenario-classification step that names the query type (growth, inactivation, transfer, mixed) is not yet on the roadmap but this query is the clearest argument we have for adding it. The audit transparency gap on temperature inference (recording matched pattern but not input phrase) is a copy-edit-level fix. The composite-food retrieval gap and the threshold calibration are both already in flight.
What this taught us: The most consequential finding is not any single technical failure but the silent scope mismatch — the system answered a different question from the one asked, with full confidence and no warning. PTM has invisible boundaries: it assumes the query is a temperature-time-pH-aw growth or inactivation problem, and any query that doesn't fit that frame gets silently coerced into one. For some queries this is harmless (B1's reject-the-load question reduces cleanly to a growth prediction the inspector can use). For others, like A4, the coercion produces a number that answers a question the user did not ask. The system would benefit from a scenario-classifier that names what kind of problem the query is (single-step growth, dynamic cooling, cross-contamination, classification, threshold comparison) and either runs the right pipeline or refuses with a clear explanation. The temperature-inference trail is a smaller but related concern: the audit is supposed to make every assumption visible, and recording only the canonical pattern that won — without showing the input phrase that triggered the match — leaves a gap in the audit chain that does not scale to scrutiny. The aw threshold near-miss is now a concrete ground-truth pair for the retrieval calibration study: query "ready-to-eat salad water activity" → expected food_properties_211 (fresh vegetables) at embedding score ≥0.61.
