...
Session state transitions go through `SessionStatus` (PENDING → EXTRACTING → STANDARDIZING → EXECUTING → COMPLETED / FAILED, or → AWAITING_CLARIFICATION when the organism clarification gate fires — see below). Grounding does not have its own status; it runs between EXTRACTING and STANDARDIZING without a status transition of its own.
