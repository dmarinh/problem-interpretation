...
- `extract_clarification_response(user_response, original_question, options=None)` — called for the first time as of A1b (2026-07-17), by `Orchestrator._resolve_organism_from_transcript()` (§5.5). `CLARIFICATION_RESPONSE_PROMPT` gives no output-format constraint, so `selected_option`/`understood_value` on the returned `ExtractedClarificationResponse` are free text, not an index or a guaranteed-exact option string — validation downstream is designed around that (loose extraction, strict acceptance; see §5.5)
...
**Organism clarification gate (A1a asks, 2026-07-16; A1b answers, 2026-07-17).** Sits between standardization and execution. An organism that can't be grounded used to always dead-end the pipeline (`missing_required` → `FAILED`) — even when the *reason* it couldn't be grounded is one a single follow-up question would resolve (the food description matched nothing, or it resolved to a category the hazard source doesn't cover). The gate narrowly targets those two cases: when `missing_required` contains the bare `"organism"` entry (not the differently-worded "grounded but not executable" case) and `GroundedValues.organism_failure.stage` is `FOOD_UNRECOGNISED` or `CATEGORY_HAS_NO_HAZARD_DATA`:

- **Round 1 (ask, no transcript on the request):** the orchestrator derives an option set — `ComBaseModelRegistry.get_executable_organisms(model_type, factor4_type)` ∩ organisms present in `pathogen_characteristics.csv`, ranked by CDC annual deaths via `GroundingService.rank_executable_organisms()`, top 5 — and asks instead of failing. `ClarificationService` (pure, deterministic, no I/O/LLM) builds the question text and options; the response carries `status=AWAITING_CLARIFICATION`, `clarification={reason, stage, question, options}`.
- **Round 2 (answer, `transcript` present on the request):** PTM has no server-side session — `SessionManager._sessions` is an unbounded, TTL-less in-memory dict, never deleted, and breaks under multiple workers, so A1b deliberately doesn't build on it. Instead `TranslationRequest.transcript` (`ClarificationTranscript`: `original_query`, `question_asked`, `options_offered`, `user_reply`) carries the round-1 exchange back on the request. The orchestrator reprocesses `transcript.original_query` from scratch through the whole pipeline and, on landing at the same gap, calls `SemanticParser.extract_clarification_response()` (its first-ever caller) to interpret the reply, then validates strictly: the resolved organism must be a *single* unambiguous match, must be among `options_offered`, and must pass a fresh `registry.is_executable()` re-check (checked, not trusted — the option set was derived at some point in the past). Any failure — skip, no match, multiple matches, an organism not offered, or a failed re-check — fails closed with a plain-language reason; it never asks a second question. On success, the organism is grounded with `source=CLARIFICATION_RESPONSE` and `standardize()` re-runs, continuing the pipeline exactly as if grounding had succeeded from the start.

Extraction is deliberately loose (the LLM's reply format isn't constrained), but acceptance is strict — the offered-set membership check and the executability re-check are the actual safety boundary, not the string-matching step. Still one round only: no multi-round, no organism default. See `app/core/orchestrator.py`'s `_handle_missing_required()` / `_resolve_organism_from_transcript()` and `app/services/clarification/clarification_service.py`.

**Multi-step duration clarification gate (2026-08-17).** A second, independent gate sharing the same seam (`_handle_missing_required()`) and one-round discipline, but a deliberately different safety model — the organism gate's safety lives in closed-set membership because its input is LLM-extracted free text; the duration gate's safety lives in a plain range check because its input is a number the user typed directly, with no LLM anywhere in the reply path. See §8.16 for why these are two gates, not two branches of the same mechanism.
...
**Ungrounded organism gets a second chance to ask (A1a, 2026-07-16):** `get_executable_organisms()`'s "for future callers" above is no longer hypothetical — the organism clarification gate is its first consumer. When organism grounding fails closed (this section) for a reason a question can resolve — `FOOD_UNRECOGNISED` or `CATEGORY_HAS_NO_HAZARD_DATA`, not the executability failure two paragraphs up, and not `BRIDGE_DISABLED`/`INTERNAL_NO_MAPPABLE_CANDIDATE` — the orchestrator derives an option set from `get_executable_organisms()` ∩ `pathogen_characteristics.csv` and asks instead of failing (`AWAITING_CLARIFICATION`, see §5.5). This does not change organism's required-field status or add a default; it only replaces a subset of dead-end failures with a question. See §5.5 for the full mechanism.
