...
- `extract_clarification_response(user_response, original_question, options=None)` — briefly called by `Orchestrator._resolve_organism_from_transcript()` as of A1b (2026-07-17), but no longer: confirmed live to return an empty extraction ~50% of the time for a clean, unambiguous answer, it was removed from the organism reply path on 2026-08-19 (see §5.5, specs/lessons.md). The method itself remains defined and unit-tested; it has no production caller.
...
*Composite-food guard (pre-Tier-1):* Before any retrieval call, `_ground_food_properties` normalises the food description and checks it against `_COMPOSITE_KEYWORDS` (a frozenset in `grounding_service.py` containing "soup", "salad", "stew", "chili", "lasagna", "gumbo", etc.) plus multi-word strings "stir-fry"/"stir fry". If a match fires, the matched keyword is recorded in `GroundedValues.composite_skip` for each field still needing grounding, and the method returns immediately — no retrieval is attempted. The orchestrator copies `composite_skip` to `InterpretationMetadata.composite_skip`; the standardization service generates a reason string naming the matched keyword; the route builder assigns `ValueSource.COMPOSITE_FOOD_DEFAULT` rather than `CONSERVATIVE_DEFAULT` for the affected fields. The guard fires before all three tiers so composite queries cannot accidentally match a single-ingredient doc at high confidence.

*Tier 3 FoodEx2 taxonomy bridge* (`_ground_via_taxonomy_bridge`, `app/services/grounding/taxonomy_bridge.py`): fires after Tier 2 for any field still ungrounded. Disabled when env var `PTM_TAXONOMY_BRIDGE_ENABLED=false` (default enabled; logged at startup). Performs deterministic fuzzy matching against `data/rag/food_taxonomy.csv` (2,917 FoodEx2 MTX entries) to resolve a `ptm_category` and `matched_state`. By the time a query reaches the bridge it has already passed the composite-food guard above. Key design decisions: (1) alias map (`data/food_taxonomy_aliases.csv`) rewrites consumer vocabulary before fuzzy matching; (2) composite scoring `(token_set_ratio, fuzz.ratio)` with tiebreaker; (3) **state-aware curated lookup** — `lookup_category_level_row(category, state, field)` looks up a single explicitly curated row from `data/rag/category_level_rows.csv` (5 rows: fresh-poultry aw, fresh-meat aw, cured-meat aw, fresh-fish pH, eggs aw); the bridge inherits ONLY from rows the source authority published as category-wide claims, never from individual-food rows. `_apply_state_default("unspecified"/"" → "fresh")` normalises taxonomy state before the lookup; the conversion is recorded as `CategoryBridgeInfo.assumed_state`. When no curated row exists for `(category, state, field)`, `GroundedValues.bridge_attempts[field]` is populated for informative defaulting downstream. Calibration (2026-05-07): threshold 80; min true-positive 90.9 ("tukey"), max true-negative 57.1 ("tornado"), gap 33.8 points.

**Reliability signals:**
- The `source` enum tier (USER_EXPLICIT / USER_INFERRED / RAG_RETRIEVAL / RAG_RETRIEVAL_FALLBACK / RAG_RETRIEVAL_CATEGORY_BRIDGE / CONSERVATIVE_DEFAULT / COMPOSITE_FOOD_DEFAULT) is the categorical reliability signal.
...
**Implementation note — model form (❓ to verify):** The main technical documentation and some code paths describe the engine as Baranyi primary with a second-order polynomial secondary. An advisory-board critique (see §11, concern #8) referred to it as "a standalone Ratkowsky implementation". Per the April advisory-review record, the authoritative implementation is Baranyi + 2nd-order polynomial secondary; a documentation/code inconsistency around Ratkowsky was surfaced and is being tracked. **This note describes `calculator.py`'s secondary model specifically and remains open** — it is unrelated to the new §5.4a Baranyi primary-model capability below, which is additive and does not touch `calculator.py`; the two questions ("what does the shipped secondary model actually implement" vs. "does a Baranyi primary model now exist anywhere in the codebase") are separate and should not be conflated when this note is eventually closed out.

### 5.4a Baranyi Primary Growth Model (additive, 2026-08-22)
**Location:** `predictive/primary/baranyi.py` — see specs/specifications.md §5.6 for the full contract (units table, h0 parameterization, inverse outcomes). Ported from a frontend TypeScript reference; not wired into the orchestrator/API — engine capability only, no `app/` consumer yet. Forward (`population_at`) and inverse (`time_to_target`, analytic, no numerical solve) both operate on explicitly-named, explicitly-based parameters (`initial_log_cfu` log10, `y_max_ln`/`mu_max_ln` already ln) to make the mixed log10/ln basis impossible to convert incorrectly by accident.
...
Session state transitions go through `SessionStatus` (PENDING → EXTRACTING → STANDARDIZING → EXECUTING → COMPLETED / FAILED, or → AWAITING_CLARIFICATION when either the organism clarification gate or the multi-step duration clarification gate fires — see below). Grounding does not have its own status; it runs between EXTRACTING and STANDARDIZING without a status transition of its own.

**Organism clarification gate (A1a asks, 2026-07-16; A1b answers, 2026-07-17; free-text-only redesign, 2026-08-19 — see specs/lessons.md).** Sits between standardization and execution. An organism that can't be grounded used to always dead-end the pipeline (`missing_required` → `FAILED`) — even when the *reason* it couldn't be grounded is one a single follow-up question would resolve (the food description matched nothing, or it resolved to a category the hazard source doesn't cover). The gate narrowly targets those two cases: when `missing_required` contains the bare `"organism"` entry (not the differently-worded "grounded but not executable" case) and `GroundedValues.organism_failure.stage` is `FOOD_UNRECOGNISED` or `CATEGORY_HAS_NO_HAZARD_DATA`:

- **Round 1 (ask, no transcript on the request):** the orchestrator asks instead of failing. `ClarificationService` (pure, deterministic, no I/O/LLM) builds the question text, naming a few common, executable pathogens in prose (e.g. "Salmonella, Listeria, E. coli, or Staphylococcus aureus") for discoverability — no options menu, no registry lookup to build one. The response carries `status=AWAITING_CLARIFICATION`, `clarification={reason, stage, question}`.
- **Round 2 (answer, `transcript` present on the request):** PTM has no server-side session — `SessionManager._sessions` is an unbounded, TTL-less in-memory dict, never deleted, and breaks under multiple workers, so A1b deliberately doesn't build on it. Instead `TranslationRequest.transcript` (`ClarificationTranscript`: `original_query`, `question_asked`, `user_reply` — free text) carries the round-1 exchange back on the request. The orchestrator reprocesses `transcript.original_query` from scratch through the whole pipeline and, on landing at the same gap, runs `transcript.user_reply` directly through `ComBaseOrganism.all_matches_in_text()` — the same deterministic substring-alias path a first-turn `pathogen_mentioned` uses, no LLM call — then validates: the resolved organism must be a *single* unambiguous match and must pass `registry.is_executable()`. Any failure — no match, multiple matches, or a non-executable organism — fails closed with a plain-language reason; it never asks a second question. On success, the organism is grounded with `source=CLARIFICATION_RESPONSE` and `standardize()` re-runs, continuing the pipeline exactly as if grounding had succeeded from the start.

This replaced an LLM round trip (`SemanticParser.extract_clarification_response()`) that was confirmed live to return an empty extraction ~50% of the time for a clean, one-word answer ("Salmonellae") across identical runs — nondeterminism on data that was never ambiguous. Deleting the LLM step also deleted the options menu, the offered-set membership check, and the LLM-vs-free-text fork: any executable organism may now be named in the reply, not just ones a prior top-5-by-CDC-deaths menu would have listed. Executability is the sole remaining safety boundary. Still one round only: no multi-round, no organism default. See `app/core/orchestrator.py`'s `_handle_missing_required()` / `_resolve_organism_from_transcript()` and `app/services/clarification/clarification_service.py`.

**Multi-step duration clarification gate (2026-08-17).** A second, independent gate sharing the same seam (`_handle_missing_required()`) and one-round discipline, but a deliberately different safety model — the organism gate's safety lives in a fresh executability check because its input is free text naming an open-ended pathogen (deterministically resolved, but not drawn from a closed set); the duration gate's safety lives in a plain range check because its input is a number the user typed directly. Neither reply path calls an LLM (organism stopped doing so on 2026-08-19 — see specs/lessons.md). See §8.16 for why these are two gates, not two branches of the same mechanism.

Fires when `missing_required` is entirely `"duration (step N)"` entries — never mixed with organism/factor4 entries, since `standardize()`'s organism/factor4 checks return before `_build_multi_step_profile()` is ever reached. This required a prerequisite fix: `_build_multi_step_profile()` used to `return None` at the *first* missing duration, so a two-step scenario with both steps vague only ever surfaced step 1. It now runs the loop to completion (every step's temperature still evaluated/defaulted/clamped) and collects every missing step before failing, so the gate can ask about all of them in one round.

- **Round 1 (ask):** the missing-step list comes from `grounded.steps` directly (`GroundedStep.duration_minutes is None`), not parsed from `missing_required` strings — `GroundedStep.duration_phrase` (the multi-step raw-text-preservation work, same session) lets the question quote the user's own words back ("step 2, you said 'a while'"). `ClarificationService.build_duration_question()` — pure, no I/O, no `options` field (duration is a number, not a menu) — builds the wording. `reason=ClarificationReason.AMBIGUOUS_DURATION`, an enum member that existed but was never wired to any caller until now.
- **Round 2 (answer):** `TranslationRequest.duration_reply` (`{original_query, steps: [{step_order, hours}]}`) — a new field, structurally separate from `transcript`, not a variant of it. `Orchestrator._resolve_duration_reply()` (a `@staticmethod` — touches no service, itself a checkable proof there's no LLM in this path) validates all-or-nothing: the reply's step_order set must exactly equal the currently-missing set (re-derived fresh each round, not echoed from round 1 — a step that resolves differently on retry shouldn't need re-answering), and each value must convert to minutes within the exact same 90-day ceiling `ExtractedDuration.value_minutes` enforces (derived programmatically from that field's own Pydantic constraint, not hand-copied). Any mismatch or out-of-range value fails closed with a plain message — never clamped, never partially applied. On success, the matching `GroundedStep`s are mutated in place (`source=CLARIFICATION_RESPONSE`, `extraction_method="clarification_direct_entry"`, `original_text` the hours the user typed) and `standardize()` re-runs.
...
- `RetrievalResult` — query, `top_match` (the doc that passed the embedding threshold and supplied the value; null when no doc cleared the gate), runners_up, `attributed_field` (str | None — set on Tier 2 per-field fallback retrievals), `reranker_top` (`SkippedDocInfo | None` — present when the cross-encoder's top-ranked result was skipped because it failed the embedding confidence gate; a lower-ranked result became top_match), `attempted_top` (`SkippedDocInfo | None` — present when all results failed the gate; top_match is null). `SkippedDocInfo` carries doc_id, content_preview, embedding_score, rerank_score, and `skip_reason` (machine-readable string, e.g. `"failed_embedding_threshold:0.70"`). The only mathematically-grounded numeric reliability signal is the embedding_score (cosine similarity).
...
**Category-level pathogen fallback (2026-05-14):** When the two-stage food-specific hazard lookup yields no result, a new fallback tier fires. It resolves food → `ptm_category` (FoodEx2 bridge, threshold 80) → IFT-2003-T1 categories (`data/rag/ift_category_alignment.csv`) → union of pathogens (`data/rag/pathogen_food_associations.csv`) → ranked by `annual_deaths_us` (`data/rag/pathogen_characteristics.csv`) → top `ComBaseOrganism`-mappable candidate. Source tier: `RAG_PATHOGEN_CATEGORY_FALLBACK`. A transparency warning is appended to `GroundedValues.warnings`. Full provenance is attached as `ValueProvenance.pathogen_category_fallback` (`PathogenCategoryFallbackInfo` in `metadata.py`). The fallback fails closed at every step — the organism stays ungrounded (not defaulted) if the bridge returns None, the category has no IFT row, or all ranked candidates have no `ComBaseOrganism` mapping. Success=False is now reserved for truly unresolvable foods (bridge miss) or unmapped categories (condiment, beverage).

**Why this decision was made:** Phase 9.5 removed the silent Salmonella default and required honest evidence for organism. The category fallback extends this principle: it provides IFT-authority-backed evidence (not assumption) for foods not in the specific hazard CSV, while preserving full audit transparency. The fallback selects Salmonella for most categories because it genuinely ranks first by CDC annual deaths — not because it's a default.
...
**A grounded organism can still fail closed: executability (Phase A0.5b, 2026-07-16):** The paragraph above covers organism *grounding* failure (no pathogen resolved at all). A second, distinct failure mode exists after grounding succeeds: the organism is real and resolved, but `data/combase_models.csv` has no row for the resolved `(model_type, factor4_type)` combination — `sf` (Shigella flexneri) is the current example, whose only row is `ModelID=1, Factor4=nitrite` (no plain-growth row, no thermal/non-thermal row at all). Previously this reached `engine.execute()`, which raised `ValueError("Model not found: sf / growth / none")` — caught by the orchestrator's generic exception handler, leaking internal short codes to the API caller with no field_audit entry. `StandardizationService.standardize()` now checks `ComBaseModelRegistry.is_executable(organism, model_type, factor4_type)` before payload construction and fails closed the same way as an ungrounded organism, but with a human-readable `missing_required` entry (`"organism ({name} is not supported for {model_type} predictions)"`). `ComBaseModelRegistry.get_executable_organisms(model_type, factor4_type)` exposes the full derived executable set (no hardcoded organism lists) for future callers. The engine's `ValueError` remains as a defensive backstop, not replaced.

**Ungrounded organism gets a second chance to ask (A1a, 2026-07-16; free-text-only redesign 2026-08-19):** when organism grounding fails closed (this section) for a reason a question can resolve — `FOOD_UNRECOGNISED` or `CATEGORY_HAS_NO_HAZARD_DATA`, not the executability failure two paragraphs up, and not `BRIDGE_DISABLED`/`INTERNAL_NO_MAPPABLE_CANDIDATE` — the orchestrator asks instead of failing (`AWAITING_CLARIFICATION`, see §5.5), naming a few example pathogens in prose. This does not change organism's required-field status or add a default; it only replaces a subset of dead-end failures with a question. `get_executable_organisms()` is no longer this gate's consumer as of the 2026-08-19 redesign — the reply is resolved deterministically against the full executable set at answer time, not from an option set derived at ask time. See §5.5 for the full mechanism.
...
### 8.15 Advisory disclosure for guesses without a conservative direction (A1c, 2026-08-17)
**Status:** ✅ Agreed.

Investigation (this phase, first prompt) audited three cases where PTM answers while guessing and asked, for each, whether the plain (non-verbose) response already discloses the guess. Finding: the top-level `warnings` list (`TranslationResponse.warnings`, unconditional — no `verbose=true` gate, §7.4/§9.1) is already the system's "we answered, but here's what we assumed" channel, and two of the three cases were already wired to it (LONG_WINDOW_DEFAULT's duration warning, and the grounding-layer `mark_ungrounded()` warning for an ungrounded temperature). The third — `_determine_model_type()`'s final fall-through branch (§5.5, `orchestrator.py`) — emitted nothing outside `combase_model.selection_reason`, which is `verbose=true`-only. This case is qualitatively different from every other default in the system: temperature/pH/aw defaults all have a conservative *direction* (worst-case-for-growth), so a wrong guess is merely cautious; growth-vs-inactivation has no such direction — it's a binary claim about what kind of scenario this is, and a wrong guess is categorically wrong, not cautious.

**What shipped (implementation only, no new mechanism):**
1. **Model-type fall-through now discloses itself.** `translate()` compares `model_type_reason` against the exact fall-through constant immediately after `_determine_model_type()` returns; on a match (and only then), appends a warning to `metadata.warnings` naming the assumption and asking the caller to confirm the scenario is storage/holding, not cooking or non-thermal preservation. Reuses the existing unconditional bridge (`metadata.warnings` → `TranslationResponse.warnings`) — no schema change. Guarded by exact string-equality against the fall-through reason (not by resulting `ModelType`), so `GROWTH` reached via explicit param, LLM inference, or any of the other seven branches never triggers it — the guard checks *how the branch was reached*, not *what it returned*.
2. **Temperature-ungrounded warning reworded, field promoted to structured.** `_ground_temperature()`'s two `mark_ungrounded()` messages ("couldn't parse the description" vs. "no description at all" — a distinction that must not collapse, see §8's structured-failure-record precedent) are now full sentences instead of a "field: reason" fragment, and deliberately omit the eventual default's numeric value — grounding runs before the orchestrator's model-type determination is passed in, and GROWTH/NON_THERMAL_SURVIVAL vs. THERMAL_INACTIVATION default to different temperatures, so naming a number here could be wrong. The route builder (`_build_warnings_list()`) now recognises the field prefix `mark_ungrounded()` always produces and splits it: `WarningInfo.field = "temperature_celsius"` structurally, message text with the prefix stripped — the same pattern the pre-existing range-clamp warning already used, just not previously applied to grounding-layer warnings.
3. **LONG_WINDOW_DEFAULT: no change.** Already fired a well-worded, self-contained warning on the plain response; verified, not touched.

**Why no new mechanism was built:** the investigation's own synthesis question — "is there an existing mechanism for this, or is A1c a wording-and-coverage problem?" — resolved to the latter for 2 of 3 cases and a true coverage gap (nothing fires) for the third. Building a new disclosure channel would have duplicated `warnings`, which already reaches the plain response unconditionally and already carries two of the three facts. See `specs/lessons.md` (2026-08-17 entry) for the pattern this generalises: check whether disclosure *infrastructure* exists before proposing to build it — the gap is often in *wiring one more emitter into it*, not in the infrastructure itself.

### 8.16 Multi-step duration clarification: a second gate, not a second branch of the first (2026-08-17)
**Status:** ✅ Agreed.

Recon (report-only phase, before any code) asked three questions before scoping this feature: does multi-step organism-ungrounded already reach the A1a/A1b gate (yes — organism resolution is entirely independent of step count, verified by a live capture, not just a code trace); what happens to a multi-step step with an unresolvable duration today (dead-ends at a hard `FAILED`, and only ever names the *first* missing step — `_build_multi_step_profile()` returned `None` at the first miss); and can the existing single-step duration resolver (`_resolve_duration_value()`) be reused as the reply-validation gate for a free-text answer. The third question's answer — no, empirically confirmed by running the actual resolver against "about two hours," "2 hours," and "120 minutes," none of which resolve (only the ~20 fixed idioms in `DURATION_INTERPRETATIONS` do; there is no numeric parser outside the LLM extraction step) — decided the whole design.

**The design decision, and why it's the opposite conclusion from organism's:** duration clarification is answered with a **structured number in hours** (`{step_order, hours}`), never free text, and never routed through an LLM. A plain number is `USER_EXPLICIT` at the same trust level as any value in the original query — there is no extraction step to distrust, so the free-text/LLM-drift problem organism's gate exists to police simply never arises for duration. This is why the two gates are **safe for different reasons**, not two branches of one mechanism: organism's safety lives in closed-set membership (an LLM's loose extraction is checked against a set it didn't produce); duration's safety lives in a plain range check (the number came from the user directly, with nothing in between to distrust). Forcing duration through organism's extract-then-match pattern would have added an LLM round-trip that bought no additional safety — the identical mistake the free-text version of this design (considered and rejected before this decision) would have made.

**What shipped, in dependency order:**
1. **Prerequisite:** `_build_multi_step_profile()` now runs to completion instead of returning at the first missing duration — every step's temperature is still evaluated/defaulted/clamped regardless, and every missing step is collected before the method fails. Without this, the gate could only ever ask about one step per round. Terminal outcome (fails closed, no duration default) is unchanged.
2. **The gate itself:** round 1 asks (naming every missing step, quoting `GroundedStep.duration_phrase` where the user said something unparseable — the multi-step raw-text-preservation work from the same session, whose only prior consumer was "nothing yet"). Round 2 answers via a new, structurally separate request field (`TranslationRequest.duration_reply`) — not an extension of `ClarificationTranscript`, which is shaped for free text and a closed offered-set that duration has no equivalent of. Validation is all-or-nothing (the reply's step_order set must exactly equal the currently-missing set, re-derived fresh each round) and range-bounded (the exact same 90-day ceiling `ExtractedDuration.value_minutes` enforces, derived from that field's own Pydantic constraint via `annotated_types` introspection rather than hand-copied as a literal that could drift).

**Recon technique worth repeating:** the third recon question ("can the existing resolver be reused") was answered by *running the actual function* against the motivating example, not by reading its code and assuming. The illustrative phrase in the original ask ("about two hours") turned out not to resolve — a second instance, in the same project, of the pattern named in the A1c session immediately prior (`specs/lessons.md`, 2026-08-17, "a bit warm" deceptively resolving via a substring rule). Two sessions in a row where an example chosen for illustration diverged from what the code actually does. Verify illustrative examples against the real function before designing around them, every time — it's cheap and it has now changed the design conclusion twice.

### 8.17 Organism clarification reply goes free-text: deleting the LLM round trip instead of hardening it (2026-08-19)
**Status:** ✅ Agreed.

Live testing confirmed the organism reply path's `SemanticParser.extract_clarification_response()` call returned an empty extraction for a clean, one-word menu label ("Salmonellae") in 2 of 6 identical runs — LLM nondeterminism on input that was never ambiguous. §8.16 (immediately above, one session earlier) had just articulated *why* organism's design was the opposite of duration's: organism's reply is free text needing extraction, so its safety was built to live in closed-set membership (extraction loose, acceptance strict). This session's finding reframes that premise — the free text didn't need LLM extraction at all. A first-turn `pathogen_mentioned` was already resolved by `ComBaseOrganism.all_matches_in_text()`, a deterministic substring-alias matcher; routing the *reply* through an LLM before reaching the same matcher was a nondeterministic round trip around a deterministic capability the codebase already had.

**The fix is a deletion, not a hardening.** Four things were removed together, each downstream of the same root cause (there was never a real need for the LLM step): the options menu (`ClarificationQuestion.options`, `ClarificationOption`), the LLM extraction call, the menu-vs-free-text fork, and the offered-set membership check (`transcript.options_offered`, `ClarificationTranscript.options_offered`). What's left: `ClarificationService.build_organism_question()` builds a question that names a few common, executable pathogens in prose (discoverability without a menu — the frontend renders the question text verbatim, so this costs no new mechanism), and `Orchestrator._resolve_organism_from_transcript()` runs `transcript.user_reply` directly through `all_matches_in_text()`, changing from an `async` LLM-calling method to a plain method (itself a small, checkable proof there's no I/O in the path — the same signal `_resolve_duration_reply`'s `@staticmethod` already used for duration). Executability (`registry.is_executable()`) is now the sole safety boundary, since there is no offered set to also check — any executable organism may be named, not only ones a prior top-5-by-CDC-deaths menu would have listed.

**Why this is the organism-specific counterpart to §8.16's duration decision, arrived at from the opposite direction:** §8.16 kept duration's reply structured-numeric specifically to avoid introducing an LLM round trip that would buy no safety. This session found that organism's *existing* LLM round trip bought no safety either — the acceptance gate (set-membership, now executability) was always the actual safety boundary, and the extraction step was just a nondeterministic detour to reach it. Same principle — a deterministic user answer (or one resolvable by a deterministic matcher) should never be routed through an LLM — applied to two designs built one session apart, reaching the same conclusion from opposite starting points: duration never added the LLM; organism removed it once its absence was shown to cost nothing.
