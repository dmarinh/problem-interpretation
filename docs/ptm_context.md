...
**Organism clarification gate (A1a, 2026-07-16).** Sits between standardization and execution. Today, an organism that can't be grounded always dead-ends the pipeline (`missing_required` → `FAILED`) — even when the *reason* it couldn't be grounded is one a single follow-up question would resolve (the food description matched nothing, or it resolved to a category the hazard source doesn't cover). The gate narrowly targets those two cases: when `missing_required` contains the bare `"organism"` entry (not the differently-worded "grounded but not executable" case) and `GroundedValues.organism_failure.stage` is `FOOD_UNRECOGNISED` or `CATEGORY_HAS_NO_HAZARD_DATA`, the orchestrator derives an option set — `ComBaseModelRegistry.get_executable_organisms(model_type, factor4_type)` ∩ organisms present in `pathogen_characteristics.csv`, ranked by CDC annual deaths via `GroundingService.rank_executable_organisms()`, top 5 — and asks instead of failing. A new `ClarificationService` (pure, deterministic, no I/O/LLM) builds the question text and options; the response carries `status=AWAITING_CLARIFICATION`, `clarification={reason, stage, question, options}`. This is ask-only: no re-entry, no answer parsing, no organism default — a future phase closes that loop. Every other missing-organism reason, and every other `missing_required` case entirely, is untouched.
