...
*Tier 3 FoodEx2 taxonomy bridge* (`_ground_via_taxonomy_bridge`, `app/services/grounding/taxonomy_bridge.py`): fires after Tier 2 for any field still ungrounded. Performs deterministic fuzzy matching against `data/rag/food_taxonomy.csv` (2,917 FoodEx2 MTX entries) to resolve a `ptm_category`, then supplies `food_properties.csv` rows for that category. Key design decisions: (1) composite-food blocklist fires before fuzzy matching — "chicken soup" would otherwise match "chicken" at score 100; (2) alias map (`data/food_taxonomy_aliases.csv`) rewrites consumer vocabulary (beef→bovine, pork→pig, lamb/mutton→sheep, veal→calf) before fuzzy matching; (3) composite scoring `(token_set_ratio, fuzz.ratio)` — token_set_ratio for recall, fuzz.ratio as tiebreaker to prevent single-word queries from matching longer substring entries by CSV order; (4) most-conservative row selection (highest upper bound per field) with `range_pending=True` so StandardizationService handles inactivation direction. When the bridge resolves a category but that category has no data for a field, `GroundedValues.bridge_attempts[field]` is populated so StandardizationService emits an informative `DefaultImputed` reason string. Calibration (2026-05-07): threshold 80; min true-positive 90.9 ("tukey"), max true-negative 57.1 ("tornado"), gap 33.8 points.
