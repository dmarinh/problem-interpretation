...
**Responsibilities:** Translate free-text user input into a typed `ExtractedScenario` using LLM + Instructor structured extraction. Also classifies intent.
...
**`extract_clarification_response()` is no longer called by the organism reply path (2026-08-19, see specs/lessons.md).** It briefly was, as of A1b (2026-07-17) — `Orchestrator._resolve_organism_from_transcript()` (§3.5) was its only caller — but was confirmed live to return an empty extraction ~50% of the time for a clean, unambiguous answer across identical runs: LLM nondeterminism on data that was never ambiguous. The organism reply path now runs `transcript.user_reply` directly through `ComBaseOrganism.all_matches_in_text()`, with no LLM call in between. The method itself remains defined (its own unit tests in `tests/unit/test_semantic_parser.py` still exercise it directly), but has no production caller. See §3.5.
...
**Side effects:** Appends `RetrievalResult` objects to `grounded.retrievals` (one per RAG call; Tier 2 fallback retrievals have `attributed_field` set). Appends warning strings to `grounded.warnings` for unresolvable fields, via `mark_ungrounded(field, reason)` (`self.warnings.append(f"{field}: {reason}")`).

**Temperature warning wording (A1c, 2026-08-17).** `_ground_temperature()` emits one of two deliberately distinct sentences when `single_step_temperature` cannot be grounded — "user said something we couldn't parse" and "user said nothing" are different epistemic states and are not collapsed into a shared template: a description present but unresolvable by rule/embedding produces `f"Temperature description '{desc}' could not be interpreted — a conservative default temperature will be assumed for this prediction."`; no description at all produces `"No temperature was specified — a conservative default temperature will be assumed for this prediction."`. Neither message names the eventual default's numeric value — `ground_scenario()` runs without knowing `model_type` (determined earlier in `orchestrator.py`, but not passed into grounding), and the two model-type families default to different temperatures (`settings.default_temperature_abuse_c` for GROWTH/NON_THERMAL_SURVIVAL vs. `settings.default_temperature_inactivation_conservative_c` for THERMAL_INACTIVATION, §3.3) — naming a specific number here could be wrong. This warning reaches the plain response the same way the pathogen-category-fallback transparency warning does (line above): via `grounded.warnings` → `metadata.warnings` (orchestrator.py, unconditional bridge) → `TranslationResponse.warnings` (§11.1). The route builder (`_build_warnings_list()`, `app/api/routes/translation.py`) recognises the `"temperature_celsius: "` prefix `mark_ungrounded()` always produces and splits it structurally: `WarningInfo.field = "temperature_celsius"`, `WarningInfo.type = "unresolved_temperature"`, `WarningInfo.message` is the sentence with the prefix stripped — rather than leaving the field name embedded in free text the way a generic warning does (`WarningInfo.field = None`). The same prefix-stripping (`_strip_temperature_warning_prefix()`) is applied when building `AuditSummary.warnings` (the verbose `audit.audit.warnings` list, §11.1) so both channels render identical wording for the same event — only the plain response additionally carries the field structurally; the verbose list remains `list[str]`.
...
- Before this phase, factor4 was the only value reaching the polynomial (`calculator.py:159`, the `b10`–`b14` terms) with no standardization pass at all — no clamp, no audit, no fail-closed check — because `engine.py` always called `calculator.calculate(clamp_to_range=False)` and `StandardizationService` never touched it. The engine's own out-of-range check (a warning-only path when unclamped) remains as a defensive backstop, not replaced. (`clamp_to_range` no longer exists as of the 2026-08-22 clamping decouple — `calculate()` always behaves as it did when called with `clamp_to_range=False`; see §5.3, §8.)
...
**Complete-then-fail on missing duration (2026-08-17).** The loop runs to completion regardless of missing durations: every step still gets its temperature evaluated/defaulted/clamped, and every step with `duration_minutes is None` is appended to `missing_required` (`f"duration (step {N})"`) — the loop uses `continue`, not `return None`, on a missing duration. Only after the full loop does the method check `if result.missing_required: return None`. This replaces the prior behavior (return `None` immediately at the *first* missing duration, leaving every later step completely unevaluated — no temperature default, no clamp, no audit entry for it). Terminal outcome for `standardize()` is unchanged either way: still `payload=None`, still no multi-step duration default. The change is purely about *how much* of `missing_required` gets populated before failing, which matters because the duration clarification gate (§3.5) needs the complete list to ask about every missing step in one round.
...
- One or more multi-step steps with an unresolvable duration, each recorded as `"duration (step N)"` — `standardization_service.py:918`. Multi-step temperature falls back to its conservative default; multi-step duration does not. As of 2026-08-17, the profile-builder loop runs to completion (see above), so this list names *every* step still missing a duration, not just the first — and, symmetrically with the organism case, this no longer always reaches `FAILED`: the orchestrator's duration clarification gate (§3.5) intercepts it whenever every entry in `missing_required` is duration-shaped (never mixed with organism/factor4 entries — see §3.5 for why that mix is structurally impossible) and asks a question instead (`AWAITING_CLARIFICATION`).
...
Consequence: `orchestrator.py` sets `SessionStatus.FAILED`, `success=False`, `error="Missing required values: ..."` — **except** the bare-`"organism"` case when the failure stage is clarifiable (A1a, §3.5) or the all-duration-entries case (2026-08-17, §3.5), either of which instead sets `SessionStatus.AWAITING_CLARIFICATION`, `success=False`, `error=None`, and one of `clarification={...}` / `duration_clarification={...}` (never both — see §3.5 on mutual exclusivity). These four `missing_required` cases remain the only reachable end-user non-success outcomes of the pipeline (excluding out-of-scope/information-query intent classification and unhandled exceptions) — three of the four reachable with today's CSV, the factor4-bounds case reachable only under a future engine's data.
...
*`Orchestrator._build_organism_clarification()`* (round 1; simplified to free-text-only 2026-08-19, see specs/lessons.md): assembles the inputs `ClarificationService` needs (it does no I/O or registry access itself) and returns `None` (→ unchanged failure path) when the stage isn't clarifiable:
1. Reads `grounded.organism_failure` (stage, `resolved_category`).
2. Delegates wording to `ClarificationService.build_organism_question()` (`app/services/clarification/clarification_service.py`) — pure, deterministic, no I/O, no LLM, unit-testable in isolation. Raises `ValueError` if called with a stage it doesn't handle. The question names a few common, executable pathogens in prose for discoverability; there is no options menu, so no registry/`get_executable_organisms()`/`rank_executable_organisms()` lookup happens here anymore — any executable organism may be named in the reply, not just ones a prior menu would have listed.
...
**Re-entry (A1b, 2026-07-17; free-text-only redesign 2026-08-19, see specs/lessons.md) — stateless, one round.** PTM keeps no server-side session (`SessionManager._sessions` is an unbounded, TTL-less in-memory dict with no caller for `delete_session()`, and it breaks under `--workers > 1` — see specs/lessons.md; deliberately not built on). Instead, `TranslationRequest.transcript: ClarificationTranscript | None` carries the round-1 exchange back on the request: `original_query`, `question_asked`, `user_reply` (free text — no `options_offered` to echo back, since there is no menu). All three fields are length-capped (max 2000 chars each); the model is a single object with scalar fields, not a list of turns, so a second round has no representable slot to accumulate into. When `transcript` is present, `Orchestrator.translate()` reprocesses `transcript.original_query` from scratch (not `user_input` — there is nothing to resume) through the *entire* pipeline, exactly as round 1 did, and only engages re-entry logic if it lands on the same clarifiable organism gap.

*`Orchestrator._resolve_organism_from_transcript()`:* a plain (non-async) method — no LLM call anywhere in this path. `transcript.user_reply` is run directly through `ComBaseOrganism.all_matches_in_text()` (§3.2/§14) — the same deterministic substring-alias path a first-turn `pathogen_mentioned` uses. This replaced a round trip through `SemanticParser.extract_clarification_response()` (§3.1), confirmed live to return an empty extraction ~50% of the time for a clean, one-word answer ("Salmonellae") across identical runs — LLM nondeterminism on data that was never ambiguous. Fails closed — returns `organism=None` with a plain-language `failure_reason` — rather than guessing, on any of:
1. The reply names zero organisms. This also covers a skip/refusal ("I don't know") — there is no separate `wants_to_skip` flag to detect, since there is no LLM extraction step left to produce one; an empty match is the whole mechanism.
2. The reply names more than one distinct organism — ambiguous, not resolved to an arbitrary one of them.
3. The resolved organism is not executable for `(model_type, factor4_type)` via `registry.is_executable()` — the actual safety boundary. There is no offered-set membership check anymore: any executable organism may be named in the reply, not just ones a prior menu would have listed.
...
`_determine_model_type()` returns `tuple[ModelType, str]`, not a bare `ModelType`. Each of the eight branches above constructs and returns its own reason string inline (some are static, e.g. `"scenario flag: is_cooking_scenario"`; others are f-strings embedding the triggering value, e.g. `f"temperature heuristic ({temp}°C > 50°C → thermal inactivation)"`) — there is no separate reason-formatting step. The caller unpacks both into `effective_model_type, model_type_reason` and passes `model_type_reason` into `ComBaseModelAudit.selection_reason`.

**Fall-through disclosure (A1c, 2026-08-17).** Branch 8's `selection_reason` (`_MODEL_TYPE_FALLTHROUGH_REASON`, the literal string `"default (no thermal/non-thermal signals detected)"`) previously reached the caller only via `ComBaseModelAudit.selection_reason`, which is `verbose=true`-only. Unlike every other default in this system, this one has no conservative direction — growth vs. inactivation is a binary choice about scenario kind, not a worst-case floor/ceiling — so a wrong guess here is categorically wrong, not cautious (see §9). `translate()` now compares `model_type_reason` against that exact constant immediately after `_determine_model_type()` returns and, only on an exact match, appends a warning to `metadata.warnings`: *"No thermal or non-thermal signal was found in this scenario — growth was assumed by default. This is a guess, not a decision: confirm this is a storage/holding scenario, not a cooking or non-thermal preservation scenario, before relying on this prediction."* Because `metadata.warnings` feeds `TranslationResponse.warnings` unconditionally (`_build_warnings_list()`, §11.1), this reaches the plain (non-verbose) response through the same channel `LONG_WINDOW_DEFAULT` already used (§9.2). The check is string-equality against the exact reason constant, not a check on the resulting `ModelType` — branches 1–7 can and do return `GROWTH` too (explicit param, LLM inference, none of the thermal/non-thermal signals firing) without matching the fall-through string, so the warning fires only on the true fall-through, never on a confident `GROWTH` decision.

**Removed branch (2026-07-16):** A ninth branch — "preservatives present (nitrite, lactic acid, acetic acid) → `NON_THERMAL_SURVIVAL`" — previously sat between 7 and the default. It was deleted: `data/combase_models.csv` has zero `Factor4` rows for `ModelID=2` (`THERMAL_INACTIVATION`) or `ModelID=3` (`NON_THERMAL_SURVIVAL`) for any organism — `ComBaseModelRegistry.is_executable()` (§5.4, Phase A0.5b) is `False` for every organism at `NON_THERMAL_SURVIVAL` with any preservative `factor4_type`. The branch could only ever route to a guaranteed `StandardizationService` refusal, converting an answerable growth question into "no model exists." Preservative presence (nitrite/lactic/acetic) still reaches `StandardizationService` unchanged via `_get_factor4()` regardless of `model_type` — only the *model-type inference* branch was removed, not the factor4 signal itself. Pathogen presence in a preserved food is now, by default, treated as a `GROWTH` question (`GROWTH`×`NITRITE` has 3 executable organisms); `pH < 4.5` and `aw < 0.90` (branches 6–7) are unaffected and still route to `NON_THERMAL_SURVIVAL`, which is executable for `LISTERIA_MONOCYTOGENES`/`SALMONELLA` at `Factor4=NONE`.

**Caveat — the LLM extraction prompt carries the same assumption the removed branch did.** `SCENARIO_EXTRACTION_PROMPT` (`app/services/extraction/semantic_parser.py`) instructs `implied_model_type="non_thermal_survival"` for preservative presence, with a worked example naming nitrite/curing directly ("Adding nitrites to cured meat" → `implied_model_type="non_thermal_survival"`). Since priority 2 (LLM inference) is checked before this removed branch ever would have been, most real nitrite-mentioning queries reach `NON_THERMAL_SURVIVAL` via the LLM's own extraction, not via priority 8 — meaning this fix corrects the branch that was provably dead-end, but does not by itself change the outcome for the common case where the LLM populates `implied_model_type` as the prompt instructs. The prompt was deliberately left unchanged in this phase.

**Multi-step duration clarification gate (2026-08-17).** A second, independent clarification gate alongside the organism gate above — same seam (`_handle_missing_required()`, between standardization and execution), same one-round-only discipline, but a structurally different safety model: organism's reply is LLM-extracted free text validated by closed-set membership; duration's reply is a plain user-typed number validated by a range check, with no LLM anywhere in the path. See §14 and specs/lessons.md for why the two gates are safe for different reasons, not variants of the same mechanism.

*Trigger:* fires when `std_result.missing_required` is non-empty and *every* entry starts with `"duration (step "` (`duration_missing_specs`, `orchestrator.py`). Mutually exclusive with the organism gate by construction: `standardize()`'s organism check (and the factor4 check) `return result` immediately on failure, strictly before `_build_multi_step_profile()` is ever called (§3.3) — so `missing_required` can never mix organism/factor4 entries with duration entries. The 2026-08-17 complete-then-fail change to `_build_multi_step_profile()` (§3.3) is a prerequisite: without it, `missing_required` would only ever contain the *first* missing step, and the gate could only ever ask about one step per round.

*Round 1 (ask):* `Orchestrator._handle_missing_required()` re-derives the missing-step list from `grounded.steps` (`[gs for gs in grounded.steps if gs.duration_minutes is None]`, sorted by `step_order`) — structured, not parsed from the `missing_required` strings, and provably identical to `duration_missing_specs` since both come from the same underlying condition. Each step is wrapped in a `DurationClarificationStep(step_order, duration_phrase)` — `duration_phrase` from `GroundedStep.duration_phrase` (the multi-step raw-text-preservation work, same session) — and passed to `ClarificationService.build_duration_question()` (pure, deterministic, no I/O, no `options` field — duration is an open numeric quantity, not a closed menu). Question wording quotes the phrase where present (`step 2 (you said "a while")`) or names the step number alone where the user said nothing. `reason=ClarificationReason.AMBIGUOUS_DURATION` — a member that existed in the enum since before this phase but was never wired to any caller until now. On fire: `state.duration_clarification_question` set, `status → AWAITING_CLARIFICATION`, a `ClarificationRecord(turn_number=1, user_response=None)` appended — same shape as the organism ask path, different field.

*Round 2 (answer):* `TranslationRequest.duration_reply: DurationClarificationReply | None` — structurally separate from `transcript` (`ClarificationTranscript`), not a variant of it: `{original_query, steps: [{step_order, hours}]}`. No `options_offered`-equivalent — unlike organism's offered set (itself a safety boundary), "which steps are missing" is recomputed fresh from `grounded.steps` every round rather than trusted from round 1, since a step that resolves differently on retry legitimately stops needing an answer. `Orchestrator._resolve_duration_reply()` (`@staticmethod` — touches no instance state, a checkable proof it calls no service/LLM) validates:
1. **All-or-nothing set match:** `{step_order in reply} == {step_order currently missing}` exactly — under-covering (fewer steps answered) and over-covering (a stale/extra `step_order`) both fail closed identically. No partial application either way.
2. **Range, per step:** `hours * 60` must be in `(0, _MAX_DURATION_MINUTES]`, where `_MAX_DURATION_MINUTES` is derived (via `annotated_types`, not hand-copied) from `ExtractedDuration.value_minutes`'s own `Field(le=131400)` — the exact same 90-day ceiling the original query extraction enforces, so a clarification reply gets no looser or stricter a bound. Out-of-range fails closed with a plain message naming the offending step and value — **never clamped**.

On success: for each answered step, the matching `GroundedStep` in `grounded.steps` is mutated in place (`duration_minutes = hours * 60`, `dur_provenance = ValueProvenance(source=CLARIFICATION_RESPONSE, original_text=str(hours), extraction_method="clarification_direct_entry")`) — `original_text` is the number the user actually typed (in hours), not the converted minutes, so the audit shows what they entered. The step-qualified `metadata.provenance`/`state.grounded_values` entries (bridged once at grounding time, stale `MISSING` placeholders since `_record_missing_required()` ran on round 1) are overwritten to match. `standardize()` re-runs on the mutated `grounded`; success continues the pipeline exactly as if every duration had resolved from the start. If the re-run still has `missing_required` (defense in depth — not reachable via the normal flow, since every answered step was just validated), it's a hard failure, never a second question — same one-round discipline as organism.

**Audit-list dedup on re-entry (caught by review, fixed same phase).** `translate()`'s first `standardize()` call unconditionally extends `metadata.defaults_imputed`/`range_clamps`/`warnings` (§7.2's post-standardization audit capture). Unlike organism — whose first call always fails at the organism check, before ph/aw/inoculum/factor4 are ever processed, so its first-call audit lists are always empty going into re-entry — duration's first call runs *past* all of that (only the multi-step profile fails), so its first-call `defaults_imputed`/`range_clamps`/`warnings` are already populated. The re-entry's second `standardize()` call therefore merges by content equality (`if d not in state.metadata.defaults_imputed: ...`, `DefaultImputed`/`RangeClamp` have no identity/timestamp fields, so `==` is reliable) rather than a plain `.extend()`, which would otherwise double-count every field that succeeded on the first call (e.g. `initial_inoculum_log_cfu`, always defaulted when not user-supplied). Organism's re-entry extend is a plain, undeduped `.extend()` and is correct as-is — its first-call lists are always empty, so there is nothing to double-count.
...
| `clarifications` | `list[ClarificationRecord]` | Shared by both clarification gates (§3.5) — the record shape doesn't distinguish which gate produced it beyond `reason`. **Organism (A1a/A1b):** round 1 (ask, no transcript) appends `ClarificationRecord(turn_number=1, reason, question_asked, user_response=None)`; round 2 (transcript present) appends exactly one record for the whole exchange — `question_asked` from the transcript, `user_response` the raw reply, `extracted_value` the resolved organism's code (or `None` on fail-closed). **Duration (2026-08-17):** round 1 appends the same ask-shaped record (`reason=AMBIGUOUS_DURATION`, `question_asked` from the freshly-built question, `user_response=None`); round 2 (`duration_reply` present) appends one record with `user_response` a `"step N: Xh; ..."` summary of the reply and `extracted_value` the same summary in minutes (or `None` on fail-closed). Never two records for one exchange in either gate — there is no session to "update in place" across requests, so round 2 constructs the complete record fresh. `default_used` is always `False` for both — neither gate has a default fallback |
...
As of the 2026-08-22 clamping decouple, `ComBaseModelConstraints` (`predictive/engines/combase/models.py`) exposes range **facts only**: the eight bound fields (`temp_min`/`temp_max`/`ph_min`/`ph_max`/`aw_min`/`aw_max`/`factor4_min`/`factor4_max`) and four validity predicates — `is_temperature_valid()`, `is_ph_valid()`, `is_aw_valid()`, `is_factor4_valid()`. There is no clamp method on this class and no clamp action anywhere in `predictive/` — the engine reports whether a value is in range and what the bounds are; it does not decide what to do about an out-of-range one. That decision (clamp, and how) is PTM's alone.

Clamping is applied by StandardizationService before payload construction, for all four scalar inputs (temperature, pH, aw, factor4) via a single shared helper, `_clamp_to_constraints()` (see §3.3 Operations 1–3b), plus one hand-inlined call site for the multi-step temperature path. Both compute the clamped value themselves — `max(valid_min, min(value, valid_max))` — using the bounds `ComBaseModelConstraints` already exposed; there is no engine method to call for the result. `ComBaseCalculator.calculate()` still validates ranges (unconditionally) and still appends a warning when a value is out of range, but it assumes the input is already valid — it never clamps. It has no `clamp_to_range` parameter (removed along with the corresponding dead internal branches, which no production caller ever reached: `engine.py`'s sole call site never passed anything but the now-default behaviour).
...
1. StandardizationService's shared `_clamp_to_constraints()` helper computes the boundary value itself — `max(valid_min, min(value, valid_max))` — using the bounds read from `ComBaseModelConstraints` (`constraints.temp_min`/`temp_max`, etc.). As of the 2026-08-22 clamping decouple, the engine exposes no clamp method to call — `ComBaseModelConstraints` provides only the bound fields and the `is_*_valid()` facts; the arithmetic is PTM's
...
The model is evaluated at the clamped value. No extrapolation occurs. (Before 2026-07-17, factor4 was the sole exception: it reached the polynomial unclamped, because `engine.py` always called the calculator with `clamp_to_range=False` and StandardizationService never touched factor4 at all — an out-of-range factor4 value produced only a warning string from the calculator and was evaluated raw, extrapolating past the model's fitted domain with no `RangeClamp`, no `range_clamps` entry, and no field-level audit signal beyond the warning. See §3.3 Operation 3b. `clamp_to_range` itself was removed from `calculate()` on 2026-08-22 — the branch it gated was reached by no production caller, proven by grepping every call site before deletion — so this paragraph describes pipeline history, not a parameter that still exists.)
...
```json
{
  ...
  "duration_reply": {
    "original_query": "string (1-2000 chars)",
    "steps": [{"step_order": "int >= 1", "hours": "float > 0"}]
  }
}
```
`transcript` (`ClarificationTranscript | None`, A1b, 2026-07-17; free-text-only redesign 2026-08-19, see specs/lessons.md) is present only when this request answers a prior `status=awaiting_clarification` response. PTM is stateless — the caller carries the round-1 exchange verbatim (echoing `question` from that response, plus the user's free-text reply — no `options_offered`, since there is no menu); `original_query` is what actually gets reprocessed, not `query` (which is still required by the schema but not otherwise used when `transcript` is present). Omit on a first-turn request.

`duration_reply` (`DurationClarificationReply | None`, 2026-08-17) is the duration gate's re-entry field — structurally separate from `transcript`, not a variant of it: a duration answer is a plain number, never free text, and never touched by an LLM (see §3.5, §14, specs/lessons.md). Present only when answering a prior `status=awaiting_clarification` response whose `duration_clarification` block was set. No `options_offered`-equivalent — the currently-missing step set is recomputed fresh from `grounded.steps` each round rather than echoed back. `hours` is enforced `> 0` at the schema level (basic sanity, a malformed request 422s); the 90-day/2190-hour plausibility ceiling is enforced internally in `Orchestrator._resolve_duration_reply()` so an out-of-range-but-well-formed reply gets a normal fail-closed `TranslationResponse`, not a generic 422. Mutually exclusive with `transcript` in practice (the two gates never fire on the same failure), though both parameters are always accepted — which one applies is decided by which gate the request actually lands on, not by which field the caller populated. Omit on a first-turn request.
...
| `clarification` | When `status=awaiting_clarification` (organism gate) | `ClarificationInfo` (2026-07-16, A1a; free-text-only redesign 2026-08-19): `reason`, `stage`, `question` — no `options` field. `question` names a few example pathogens in prose for discoverability; the frontend renders a free-text input, not a menu. `error` is `None` in this case — `state.set_error()` is not called, so this is not treated as an error. The client echoes `question`→`question_asked` inside the follow-up request's `transcript` (A1b), plus the user's free-text `user_reply`. |
| `duration_clarification` | When `status=awaiting_clarification` (duration gate) | `DurationClarificationInfo` (2026-08-17): `reason` (always `ambiguous_duration`), `question`, `steps` (list of `{step_order, duration_phrase}` — `duration_phrase` is `null` when the user said nothing about that step's duration at all). `error` is `None`, same convention as `clarification`. Mutually exclusive with `clarification` — never both non-null on the same response (§3.5). No `options` field: answer via `TranslationRequest.duration_reply`, a structured `{step_order, hours}` list, not a selection from this block. |
...
**Organism clarification instead of failure (A1a, 2026-07-16; free-text-only redesign 2026-08-19, see specs/lessons.md):** when the bare `"organism"` missing-required case's `organism_failure.stage` is `FOOD_UNRECOGNISED` or `CATEGORY_HAS_NO_HAZARD_DATA`, the response is `status=AWAITING_CLARIFICATION, success=False, error=None, prediction=None, clarification={...}` instead of the generic missing-required error above — no viable-option-set check anymore, since there is no option set to derive. Example (unrecognised food): `question="I don't recognise \"frobnitz\" as a food I have safety data for, so I can't infer which pathogen to model. Please tell me directly which pathogen you're concerned about (for example Salmonella, Listeria, E. coli, or Staphylococcus aureus) — rephrasing the food description might also help."` — no `options` field.

**Re-entry fail-closed responses (A1b, 2026-07-17; free-text-only redesign 2026-08-19):** when `transcript` is present and `_resolve_organism_from_transcript()` (§3.5) fails, the response is a plain `success=False, status=FAILED` (never a second `AWAITING_CLARIFICATION`), with `error` naming what was said and why it couldn't be used — never a short code:
- Reply names no pathogen (including a skip/refusal like "I don't know" — there is no separate skip flag anymore): `"Your reply ('...') didn't name a pathogen I could identify, so this can't proceed."`
...
- Resolved to a single, unambiguous organism, but it fails the executability check for `(model_type, factor4_type)`: `"{name} is not supported for {model_type} predictions, so this can't proceed."` — this is the real safety boundary now; there is no offered-set membership check to fail first.

**Duration clarification instead of failure (2026-08-17):** when every entry in `missing_required` is a `"duration (step N)"` spec (§3.3, §3.5), the response is `status=AWAITING_CLARIFICATION, success=False, error=None, prediction=None, duration_clarification={...}` instead of the generic missing-required error above. Example (two steps, one with an unresolvable phrase): `question="I need an exact duration to run this prediction, but step 1 (you said \"a while\"), and step 2 didn't resolve to a specific length. Please provide the duration in hours for each listed step."`, `steps=[{step_order: 1, duration_phrase: "a while"}, {step_order: 2, duration_phrase: null}]`.

**Duration re-entry fail-closed responses (2026-08-17):** when `duration_reply` is present and `_resolve_duration_reply()` (§3.5) fails, the response is a plain `success=False, status=FAILED` (never a second `AWAITING_CLARIFICATION`), with `error` naming what was wrong — never a short code:
- Reply doesn't cover exactly the currently-missing steps (fewer answered, or a stale/extra `step_order`): `"This reply must answer exactly the steps that are still missing a duration (step 1, 2), but named step 1 instead — this can't proceed."`
- A value outside `(0, 2190]` hours: `"Step 1: 5000.0 hours is outside the valid range (0, 2190] hours — this can't proceed."` — not clamped; nothing is applied.
...
| `ClarificationQuestion` | (2026-07-16; free-text-only redesign 2026-08-19, see specs/lessons.md) Built by `ClarificationService.build_organism_question()` — `reason` (`ClarificationReason`), `stage` (`OrganismGroundingFailureStage`), `question` text (names a few example pathogens in prose; no `options` field — there is no menu). Stored on `SessionState.clarification_question`; surfaced via `TranslationResponse.clarification` (§11.1). See §3.5. |
| `ClarificationTranscript` | (2026-07-17, A1b; `options_offered` removed 2026-08-19) The round-1 exchange, carried on `TranslationRequest.transcript` so a stateless server can complete a clarification without a session: `original_query`, `question_asked`, `user_reply` (free text). All fields length-capped; single object (not a list of turns), so it cannot represent more than one round. See §3.5. |
| `all_matches_in_text()` | (2026-07-17, A1b) `ComBaseOrganism` classmethod — same fuzzy alias matching as `from_text()`, but returns the *set* of every distinct organism found in text rather than stopping at the first match. Used to detect an ambiguous reply (more than one organism named) rather than silently picking one. As of the 2026-08-19 free-text redesign, this is the entire resolution mechanism for an organism clarification reply — called directly on `transcript.user_reply`, no LLM extraction step in between. See §3.5. |
| `DurationClarificationQuestion` | (2026-08-17) The duration gate's ask-side question: `reason` (always `AMBIGUOUS_DURATION`), `question` text, `steps` (list of `DurationClarificationStep`). No `options` field — duration is an open numeric quantity, not a closed menu. Built by `ClarificationService.build_duration_question()`, pure and deterministic. Stored on `SessionState.duration_clarification_question`; surfaced via `TranslationResponse.duration_clarification` (§11.1). See §3.5. |
| `DurationClarificationStep` | (2026-08-17) One missing step named in a `DurationClarificationQuestion`: `step_order`, `duration_phrase` (the raw phrase from `GroundedStep.duration_phrase`, or `null` if the user said nothing about that step's duration at all). |
| `DurationClarificationReply` | (2026-08-17) The duration gate's round-2 re-entry, carried on `TranslationRequest.duration_reply`: `original_query`, `steps` (list of `DurationStepReply`). Structurally separate from `ClarificationTranscript`, not a variant of it — see `ValueSource.CLARIFICATION_RESPONSE`'s entry below on why. See §3.5. |
| `DurationStepReply` | (2026-08-17) One step's answer in a `DurationClarificationReply`: `step_order`, `hours` (`Field(gt=0)` — a plain number typed by the user, never LLM-parsed). The 90-day/2190-hour plausibility ceiling is enforced internally in `Orchestrator._resolve_duration_reply()`, not at this schema level, so an out-of-range reply gets a normal fail-closed response rather than a generic 422. See §3.5. |
