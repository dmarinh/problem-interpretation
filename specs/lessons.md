...
### 2026-07-17 — A standalone script using production singletons must replicate ALL of the lifespan startup, not just the one that fails loudly

**What surfaced:** `exp_5_1_clarification_fire_rate.py`'s first live run (165 queries, real cost) reported 100% `other_failure` across every single category, including trivially-groundable foods like "chicken" and "wine". The error on every query was identical: `"VectorStore not initialized. Call initialize() first."` The experiment's `build_orchestrator()` replicated one half of `app/main.py`'s FastAPI lifespan startup (`ComBaseEngine.load_models()`, guarded by an `is_available` check that makes the unloaded state easy to detect and branch on) but not the other half (`VectorStore.initialize()`). `RetrievalService()`'s default constructor happily accepts an uninitialized `VectorStore` — construction never raises — so the gap was invisible until the first real query reached deep into `GroundingService` and threw. The entire run (~$2.35) measured nothing but this one harness bug, not the pipeline.

**Why it wasn't caught before spending money:** `--dry-run` (which was run and reviewed before approval) never touches the orchestrator or any service singleton — by design, it only reads the corpus and prints a cost estimate, so it could not have caught a service-wiring bug. The smoke test that would have caught it (one real, cheap query against a known-easy food before committing to the full batch) was skipped in favor of going straight to the full run once cost approval was granted, since the estimate itself had already been validated.

**Fix:** added a symmetric check — `if not store.is_initialized: store.initialize()` — right next to the existing `ComBaseEngine.is_available` check, both now visually paired in `build_orchestrator()`'s body so a future singleton added to this function is more likely to get the same treatment by proximity. Re-ran a single live query against "chicken" directly (bypassing the cache) to confirm the fix before re-running the full paid batch.

**Heuristic (codified):** when a script constructs production service singletons directly instead of going through the app's real startup path (FastAPI lifespan, test fixtures, etc.), enumerate *everything* that startup path does — not just the steps whose omission fails at construction time. A step that fails silently at construction and loudly (but only) on first use is the dangerous kind, because it passes every check that doesn't actually exercise the service. Before authorizing a paid/live batch run, spend one query's worth of cost on a smoke test against an easy, known-good case — a cost estimate only validates the *count*, never whether the *thing being counted* actually measures what it claims to. Grep the real startup path (`app/main.py`'s lifespan, or the equivalent) line by line against the standalone script's setup and require a 1:1 justification for every step that's intentionally skipped, not just the ones that happen to be obviously necessary.

---

### 2026-08-17 — A1c: check whether disclosure infrastructure exists before proposing to build it

**Context:** Investigated three cases where PTM answers a query while guessing (model-type fall-through with no thermal/non-thermal signal; LONG_WINDOW_DEFAULT's near-content-free 7-day trajectory; an unresolved temperature description) and whether the plain, non-verbose API response discloses each guess. The investigation report (first prompt, report-only) found that the top-level `warnings` array — unconditional, no `verbose=true` gate — was already the system's "we answered, but here's what we assumed" channel, and was already carrying two of the three facts. Only the model-type fall-through emitted nothing outside a verbose-only field (`combase_model.selection_reason`).

**What went well:**
- Reading the actual propagation path before proposing a fix prevented over-building. The natural-sounding move — "add a confidence/assumption field to the response schema" — would have been a second, redundant mechanism sitting next to one that already worked for 2/3 cases. Tracing `metadata.warnings` from `mark_ungrounded()` (grounding) and `result.warnings.append()` (standardization) through the orchestrator's unconditional bridge (`orchestrator.py:830-831`, and `:245` for standardization's own list) to `_build_warnings_list()` (always called, not gated by `verbose`) settled the question before any code was written: the mechanism existed, coverage was the gap.
- The implementation prompt explicitly scoped out a new `ValueSource` and any schema change, forcing the fix to live inside the existing channel. This produced a smaller, more surgical diff than the investigation's own three-case framing might have suggested (three problems does not imply three fixes).
- Checking a concrete example against the actual rule library before writing a test caught a wrong premise: the illustrative phrase from the original ask ("a bit warm") was assumed unresolvable, but `find_temperature_interpretation_with_fallback("a bit warm")` actually matches the substring rule `"warm"` → 30°C. Running the real function before writing an integration test around it avoided shipping a test with a false premise; `"xyz123"` (already used elsewhere in the test suite for this exact purpose) was substituted instead.

**What surfaced repeatedly:**
- **A "guess disclosure" is not a single kind of default.** Temperature/pH/aw/duration defaults all have a conservative *direction* — worst-case-for-growth — so a wrong guess there is merely cautious, and the existing `CONSERVATIVE_DEFAULT`/`LONG_WINDOW_DEFAULT` machinery already treats them as safety events. Model-type has no such direction (growth vs. inactivation is a binary claim about scenario kind, not a floor/ceiling), so a wrong guess there is categorically wrong. The fix had to key off *how the branch was reached* (exact-match against the fall-through reason string) rather than *what value it produced* (`GROWTH`), because `GROWTH` is also the confident answer on at least two other branches (explicit param, LLM inference) — matching on the resulting enum value instead of the reason string would have made the warning fire on confident paths too, exactly the "cry wolf" failure the task called out as the disqualifying outcome.
- **Wording a disclosure sometimes has to omit a fact the reader would want, because the code doesn't have it yet.** The natural wording for the temperature-ungrounded warning would name the numeric default about to be applied ("...will default to 25°C"), matching LONG_WINDOW_DEFAULT's style. But `_ground_temperature()` runs inside `ground_scenario()`, which is never given `model_type` — and GROWTH/NON_THERMAL_SURVIVAL default to a different temperature than THERMAL_INACTIVATION (`settings.default_temperature_abuse_c` vs. `settings.default_temperature_inactivation_conservative_c`). Hardcoding "25°C" into a grounding-layer message would be accurate in the common case and silently wrong in the rare one — exactly the class of bug this codebase's lessons file has flagged repeatedly (documented-as-harmless claims that are true today and not invariant). Chose a model-type-agnostic phrasing ("a conservative default temperature will be assumed") over plumbing `model_type` into grounding for one warning string — the plumbing change would have been larger than the wording task justified, and the honest-but-vaguer sentence was preferred over a precise-but-sometimes-wrong one.
- **`WarningInfo.field` was already in the schema, unpopulated for one branch.** The range-clamp branch of `_build_warnings_list()` already set `field=clamp.field_name` from a structured `RangeClampInfo` object; the general-warnings branch never set `field` because `metadata.warnings` is a flat `list[str]`. Restructuring `metadata.warnings` into a list of structured objects (to carry `field` generically) would have been the "new mechanism" the task explicitly ruled out and would have touched every append call site across grounding, standardization, and the orchestrator. Instead, `mark_ungrounded()`'s existing, stable `f"{field}: {reason}"` formatting was used as a known, parseable prefix — the route layer strips exactly `"temperature_celsius: "` and promotes it to the structured field, scoped to the one warning this phase touched. A narrow, format-dependent parse at the consumption boundary was the right size fix; broadening the producer's return type for every caller was not.

**What to do differently:**
- When a task asks "does mechanism X already do this," trace the actual data flow (grep every append site, follow it through every unconditional-vs-conditional branch to the response) before answering — the answer determines whether the next phase is "wire one more emitter in" (small) or "design a new channel" (large), and guessing wrong in either direction produces a differently-shaped implementation prompt.
- When a warning's wording depends on information the emitting layer doesn't have (here: the eventual default value, which depends on a decision made in a different pipeline stage), don't silently guess which value is "the common case" — either plumb the missing information through, or write the honest, less-specific sentence and say so explicitly in the report, rather than picking a plausible-looking number that's right most of the time.
- Before writing a test around a "this phrase should fail to resolve" premise, run the actual resolver against the actual phrase. An illustrative example in a task description is not verified data.

---

### 2026-08-17 — Multi-step organism clarification already works; recon before design prevented duplicate wiring

**Context:** Scoped a future multi-step duration-clarification feature. First question asked (report-only, no code): does a multi-step scenario with an ungrounded organism already reach the A1a/A1b clarification gate, or does it dead-end at `missing_required` the way single-step did before A1a? The answer determines whether multi-step clarification extends an existing path or requires building a new one.

**Finding, live-verified (not just read):** it already works, today, with zero multi-step-specific code anywhere in the gate. Organism resolution (`ground_scenario()`'s pathogen-grounding steps, and `standardize()`'s `organism = self._get_organism(grounded); if organism is None: ...` check) runs entirely before the `grounded.has_steps` branch that dispatches to `_build_multi_step_profile()` — organism has no concept of "which step." `Orchestrator._handle_missing_required()`'s trigger (`"organism" in std_result.missing_required` + a clarifiable `organism_failure.stage`) has zero reference to `is_multi_step`/`has_steps` anywhere in the method. A live capture (2-step "frobnitz" scenario) confirmed `status=AWAITING_CLARIFICATION` with the identical question/option shape single-step produces — not a guess from reading the code, an actual run.

**Contrast — multi-step duration is NOT similarly covered.** Unlike organism, a missing multi-step step duration (`"duration (step N)"` in `missing_required`) does *not* match the gate's bare-`"organism"` trigger, so it falls straight through to `state.set_error(...)` → hard `FAILED`, exactly the dead-end pattern A1a eliminated for organism. Also unlike single-step duration (which gets `LONG_WINDOW_DEFAULT` instead of failing), multi-step step duration has no default at all — `_build_multi_step_profile()` returns `None` on the first missing step and never processes any step after it (no temperature defaults, no audit entries for later steps).

**Why this mattered for scoping:** without tracing this first, a design could have proposed "wire multi-step into the A1a gate" as if that were new work — it isn't; organism is already fully general. The actual gap is duration, which is a structurally different problem (a value-level default/clarification, not an organism-resolution problem) and needs its own design, not an extension of the organism gate's trigger condition.

**Heuristic (codified):** before designing a feature that sounds like "extend gate X to cover case Y," trace whether X's trigger condition has any structural dependency on the dimension Y varies along (here: single-step vs. multi-step). If the trigger is defined purely in terms of a field name and a failure stage — as A1a's is — it may already be general across a dimension nobody explicitly generalized it for, because nobody needed to. Confirm with a live run, not a read-through; a code trace establishes *reachability*, a captured response confirms *actual behavior*, and the two occasionally diverge on branches this rarely exercised (multi-step + organism failure is not in the existing test suite at the time of this recon).

**Separate finding — raw user phrasing is read-then-discarded in `_ground_multi_step_profile()`.** `step.duration.description` (and `step.temperature.description`) are read to build the human-readable half of `grounded.warnings`' unresolved-value message (`f"Step {order} duration: Could not interpret: '{desc}'"`), then never passed to `GroundedStep` — `add_step()`'s signature simply had nowhere to put them. The phrase itself survives fine up to that point (`ExtractedTimeTemperatureStep.duration.description`, populated by the LLM extraction, untouched by anything between the parser and grounding) — the discard is entirely local to `_ground_multi_step_profile()`, not an upstream data-loss problem. Fix: two new optional fields on `GroundedStep` (`duration_phrase`, `temperature_phrase`), populated unconditionally (not just on the unresolved path — a resolved-via-rule phrase like "overnight" is still worth keeping for the same reason), purely additive, no consumer yet. The general pattern (also named in the 2026-05-07 and 2026-07-15 entries above): a value used to build a *string* for a human is not the same as a value *available* to downstream code — if nothing carries the raw value forward in structured form, it's gone the moment the string is built, no matter how trivially available it was one line earlier.

---

### 2026-08-17 — Multi-step duration clarification: structured numbers, not free text — two gates safe for different reasons

**Context:** Follow-on to the recon above. Before scoping the duration gate, three more questions were asked and answered by report before any code: (1) what's the request/reply contract — can a duration reply ride the same `ClarificationTranscript` field organism uses, or does it need its own; (2) what's the multiplicity outcome when several steps are missing and the reply only answers some of them; (3) is there a structured, string-free trigger for "which steps are missing," now that `GroundedStep.duration_phrase` exists.

**The design decision — and why it's the *opposite* conclusion from organism's, on purpose:** a duration reply is a structured number in hours (`{step_order, hours}`), never a sentence, never routed through an LLM. This was decided, not defaulted into — the natural-looking move (mirror A1b: LLM-extract the reply, then validate) was checked and rejected. `_resolve_duration_value()` (the existing single-step resolver) was the obvious thing to try reusing as the validation gate. Running it directly against the ask's own worked example — "about two hours" — showed it returns `None`: the rule table it delegates to (`DURATION_INTERPRETATIONS`, `app/config/rules.py`) is a fixed set of ~20 idioms with zero numeric parsing; "2 hours" and "120 minutes" fail to resolve too. Single-step duration only handles these in production because the *original query's* LLM extraction pass converts them before this resolver ever sees them — the resolver itself never parses numbers out of text. Reusing it as a free-text reply gate would have meant either (a) adding a second LLM extraction call duration never needed before, or (b) accepting that the gate rejects most well-formed numeric answers. Neither is a validation gate; (b) is a refusal machine wearing a validation gate's clothes.

**The reframe that resolved it:** stop asking "how do we validate a free-text duration reply" and ask "does the reply need to be free text at all." It doesn't — a clarification reply is a fresh, small, purpose-built interaction, not a constrained rephrasing of the original query. Asking for a number in hours costs nothing a real user would object to, and it deletes the entire extraction-trust problem: a plain number the user typed directly is `USER_EXPLICIT` at the same trust level as any value in the original query, because there is no LLM step between the user's intent and the value inside the pipeline.

**Named pattern: two gates, safe for different reasons, not two branches of one mechanism.** Organism's safety property lives in *acceptance*: an LLM's loose extraction is checked against a closed, caller-supplied set the LLM didn't produce (`options_offered` membership + a fresh `registry.is_executable()` re-check) — the check doesn't get weaker no matter how loose the extraction is, because the extraction was never where the safety lived. Duration's safety property lives in *validation of a plain value*: there is no extraction step to distrust, so the check is a structural range bound, full stop. The temptation this session named explicitly: seeing "gate #2, similar shape to gate #1" and reaching for "reuse gate #1's mechanism" as if similarity in *shape* (ask a question, get a reply, validate, apply-or-fail-closed) implied similarity in *what makes it safe*. It didn't. Forcing duration through organism's extract-then-match pattern would have been the same category error as the earlier `_category_pathogen_fallback()` "reuse the ranking logic" case (2026-07-16 entry above) — copying a mechanism built for a different input shape instead of identifying which *property* was meant to be shared. Here, nothing was meant to be shared beyond "ask once, validate strictly, fail closed, one round only" — the validation *mechanism* is legitimately different because the *trust boundary* is different.

**Multiplicity: all-or-nothing, decided before implementation, not discovered by testing.** When several steps are missing, a reply covering only some of them is rejected outright — nothing is partially applied, even though the partial answer is individually valid. Reasoning, decided in the report-only phase: (1) no precedent anywhere in this codebase's clarification design for partial acceptance — organism's checks are all all-or-nothing gates before any `grounded.set()` call; (2) A1b's one-round-only discipline means a partial fill has no second round to be useful in — the user still can't get a prediction either way, so partial acceptance buys nothing and costs an audit state that's confusing to explain (some steps `source=clarification_response`, pipeline still failed); (3) it composes as a single pre-check (`{reply step_orders} == {currently-missing step_orders}`) computable before touching `grounded` at all, mirroring organism's pre-checks exactly. Also handles the reverse mismatch (a stale/extra `step_order` from an earlier round) identically to an incomplete one, with no separate code path.

**Completeness prerequisite, and the reason a prior "keep this behavior" recommendation reversed.** `_build_multi_step_profile()` returned `None` at the *first* missing duration (a deliberate design choice from the original `LONG_WINDOW_DEFAULT` phase, recorded in `SPEC_long_window_default_duration.md:413` as "Recommendation: keep this behavior unchanged... Adding iteration to collect all missing steps would be a disproportionate change" — correct at the time, since nothing consumed a complete list). The duration gate is the first consumer that needs the complete list, so the loop now runs to completion (`continue` instead of `return None`, check `missing_required` after the loop) — every step's temperature is still evaluated regardless of a sibling step's missing duration, which it wasn't before. Terminal outcome (fails closed, no duration default) is unchanged; only how much of `missing_required` gets populated before failing changed. Named pattern (recurring in this codebase, see the 2026-07-15/07-16/07-17 "documented as harmless" entries): **a design recommendation that was correct when written can become wrong not because the original reasoning was flawed, but because a new consumer's requirements changed what "correct" means** — the fix is to update the record when the premise changes, not to treat the old recommendation as permanently settled. `SPEC_long_window_default_duration.md` was updated in place with a dated correction note rather than silently rewritten, so the history of *why* the recommendation changed stays legible.

**Recon technique that paid for itself twice in two consecutive sessions.** The "about two hours doesn't resolve" finding is the same failure mode as the immediately-prior A1c session's "a bit warm" discovery (both in this file, both dated 2026-08-17): an illustrative example chosen for a task description, assumed to demonstrate the target behavior, actually diverges from what the real code does when you run it. Both times, running the actual function against the actual example — not reading the code and reasoning about it — caught the divergence before it shaped a wrong design. Two for two, same session-adjacent pattern, worth naming as a standing habit rather than a one-off catch: **before designing around a worked example in a task description, run it.**

**Small mypy gotchas from this implementation, worth naming so the next person doesn't rediscover them:**
- Two `@dataclass` result types (`_TranscriptResolution`, `_DurationReplyResolution`) assigned to a local variable with the *same name* (`resolution`) in different `if`-branches of the *same function* (`_handle_missing_required`) — runtime-safe (the branches are mutually exclusive), but mypy infers one type for the name across the whole function from its first assignment and flags the second branch's assignment as incompatible, plus every subsequent attribute access. Fix: distinct local names per branch (`duration_resolution`), not a shared name across mechanically-different result types even when the branches never overlap at runtime.
- Deriving a constant from a Pydantic v2 field's `Field(le=...)` constraint via `ExtractedDuration.model_fields["value_minutes"].metadata` (a list of `annotated_types.Gt`/`Le` objects — see the A1c-adjacent `_MAX_DURATION_MINUTES` derivation) returns a value typed `SupportsLe`, not `float`, even though the runtime value is a plain number. `float(cast(int, value))` — cast first to silence the structural-protocol mismatch, then call `float()` for a real runtime conversion — was needed to get both a correct static type *and* a real `float` at runtime; casting directly to `float` (`cast(float, value)`) satisfies mypy but performs no runtime conversion, silently leaving an `int` in a variable annotated `float`.

**Code review caught a real HIGH-severity bug this pattern-matching missed: double-counted audit entries on the duration gate's success path.** `translate()` calls `standardize()` once unconditionally, then unconditionally extends `metadata.defaults_imputed`/`range_clamps`/`warnings` from that first result — a pre-existing, deliberate mechanism (2026-05-07 entry above) so even a hard failure gets a complete audit. The duration gate's re-entry success path calls `standardize()` a *second* time on the same request (after applying the reply) and extended from *that* result too, with a plain `.extend()` — for organism, this is always safe, because organism's first `standardize()` call returns immediately at the organism check, before ph/aw/inoculum/factor4 are ever touched, so its first-call `defaults_imputed` is always empty and re-entry really is the *first* time those fields get recorded. Duration's first call runs *past* all of that — organism and factor4 are already resolved, only the multi-step profile fails — so its first-call `defaults_imputed` is already populated, and the second `standardize()` call re-defaults/re-clamps every already-succeeded field, silently doubling those entries in the audit (empirically reproduced: `initial_inoculum_log_cfu` appeared twice; a variant with an unset step temperature produced two of *that* too). Fix: dedupe by Pydantic `==` equality (`DefaultImputed`/`RangeClamp` have no identity/timestamp fields, so this is reliable) before appending, in the one block where the second `standardize()` call's results get merged in.

**Why this survived the "mirror A1b's re-entry structure" instruction:** the instruction was followed faithfully at the *shape* level (ask, validate, apply, re-run standardize, extend audit lists) — the bug is that the *safety* of that shape, for the audit-list-extend step specifically, depended on an invariant (first call's audit lists are always empty) that holds for organism but not for duration, for the exact same reason the validation *mechanism* had to differ (documented above): duration's failure point in `standardize()` is structurally later than organism's. "Mirror the structure" is not the same guarantee as "mirror the safety properties" when the two call sites diverge in *where in the pipeline* they fail — every point where one gate's mirrored code depends on an invariant true only because of *where* the original gate's failure happens needs to be checked against where the new gate's failure actually happens, not assumed to carry over. This is the same lesson as "two gates, safe for different reasons" one level down: shape-mirroring can silently import an assumption that was never meant to travel with the shape.

---

### 2026-08-19 — Organism clarification reply goes free-text: a deterministic user choice should never be routed through an LLM

**Context:** Confirmed live, two sessions after the entry immediately above: the organism reply path's `SemanticParser.extract_clarification_response()` call returned an empty extraction for a clean, one-word menu label ("Salmonellae") in 2 of 6 identical runs. The label wasn't ambiguous, wasn't malformed, wasn't adversarial — it was the exact text the system itself had offered a moment earlier. The failure was pure LLM nondeterminism on data that was never in question.

**The fix was a deletion, not a hardening.** The instinct when an LLM step is flaky is usually to add retries, tighten the prompt, or add a validator pass. All three would have treated the LLM call as load-bearing and tried to make it more reliable. The actual fix removed it: `Orchestrator._resolve_organism_from_transcript()` now runs `transcript.user_reply` straight through `ComBaseOrganism.all_matches_in_text()` — the exact deterministic substring-alias matcher a first-turn `pathogen_mentioned` already used, sitting one call away the whole time. Four things fell out once the LLM step was gone, all downstream of the same root cause: the options menu (`ClarificationQuestion.options`, `ClarificationOption` — nothing to build), the menu-vs-free-text fork, the offered-set membership check (nothing to check membership against), and `_resolve_organism_from_transcript()`'s own `async`-ness (no I/O left in the method at all). One nondeterministic bug killed, four pieces of now-unnecessary machinery removed, one already-existing code path reused instead of duplicated.

**The safety boundary didn't move — it was already there.** Before this change, organism's actual safety lived in the offered-set membership check plus the executability re-check; the LLM extraction feeding it was explicitly *designed loose* (see the 2026-07-17 entry's "extraction loose, acceptance strict" framing) specifically because the team already knew not to trust the extraction step. That framing was correct as far as it went, but it stopped one inference short: if the acceptance gate is already doing all the safety work and tolerates arbitrarily loose input, the extraction step isn't earning its complexity or its failure mode — it's just a slower, flakier way to get text into the gate that already has to validate it strictly regardless. Removing the offered-set half of the gate (there's no menu to be a subset of anymore) didn't weaken safety either: executability (`registry.is_executable()`) was always the check that actually mattered, since an organism named in a menu was never guaranteed safe by having been *offered* — it was safe because it was executable, checked fresh, every time.

**Heuristic (generalizes beyond this bug):** before adding an LLM extraction step in front of a validation gate, ask whether the gate would work directly on the raw input. If the answer is "yes, and here's the deterministic function that already does it," the extraction step is not making the system smarter — it's inserting a nondeterministic round trip between the user and a check that was going to run anyway. This is easiest to miss when the deterministic matcher was written for a *different* caller (here: first-turn scenario extraction) and nobody asks "does the reply path already have access to the same tool."

**Named as the organism-specific counterpart to the duration decision, arrived at from the opposite direction** (see the two 2026-08-17 entries above, "two gates, safe for different reasons"): duration was designed *never* to add an LLM round trip, having checked and found no existing resolver would substitute for one honestly. Organism *had* an LLM round trip already in production, and this session found — by testing the live behavior, not by re-reading the design rationale — that it had never been necessary. Same principle, opposite starting conditions: never build the round trip if a deterministic path already reaches the safety gate; delete the round trip once it's shown to be removable without any loss of safety. A deterministic user choice, or a reply resolvable by a deterministic matcher the codebase already has, should never be routed through an LLM.

---

### 2026-08-22 — Pre-refactor characterization tests: an unverified docstring claim caught by the same discipline it was describing

**Context:** Added characterization tests (test-only, no production code changed) ahead of the planned engine-relocation refactor: value-pinned polynomial math for three real (organism, model_type) rows, a full-response snapshot baseline for four scenarios, a pH clamping "full triple" test to match the existing temp/aw/factor4 ones, and enum member-set pins for `ModelType`/`Factor4Type`/`EngineType`.

**What went well:**
- The polynomial characterization tests were built from an independent standalone script (plain arithmetic, no import of `ComBaseCalculator`) rather than trusting the calculator's own output — then cross-checked against the real calculator only as a sanity check, not as the source of the expected value. "Teeth" were verified empirically, not assumed: a coefficient was deliberately perturbed in `data/combase_models.csv`, the targeted test failed with the expected mismatch, then the file was restored and reconfirmed clean via `git status` producing no diff. The same empirical-teeth check was repeated for the snapshot fixture (perturbed one field in `expected_outputs.json`, confirmed the test failed with a divergence assertion, reverted).
- The snapshot baseline runs through the real FastAPI route (`monkeypatch`ing `get_orchestrator` in the route module, not calling the orchestrator directly), so the response-building logic in `translation.py` is pinned too, not just the orchestrator's internal state — this was a deliberate choice over the simpler "call `orchestrator.translate()` and assert on the result object," because "full response body" per the task meant what a client actually receives, verbose audit blocks included.

**What surfaced — an unverified claim, caught before it shipped:** the snapshot test's first-draft docstring asserted "every retrieval query misses... so pH/aw fall through to conservative defaults deterministically." Reading the actual captured response's `provenance` block showed `water_activity` resolving via `source: rag_retrieval_category_bridge` (a curated, CSV-derived category fallback), not `CONSERVATIVE_DEFAULT` — the claim was subtly wrong about *which* deterministic path fired, even though the determinism conclusion the test actually depends on was still correct. This is the same shape as the "documented-or-coded-as-safe, refuted by a trace" pattern this file has logged repeatedly (see the 2026-07-17 factor4 entry, the bl/bt enum swap, the Salmonella default) — a plausible-sounding claim about system behavior, written without tracing the actual code path, that happened to be checkable in under a minute. It was checked before being committed, not after. Fixed by (a) correcting the docstring to name both possible deterministic paths instead of asserting one specific mechanism, and (b) actually re-running the capture a second time and diffing byte-for-byte against the first capture (only the three masked fields — `session_id`/`created_at`/`completed_at` — differed) to verify the determinism claim itself, rather than asserting "confirmed empirically" without having confirmed it.

**Heuristic (reinforces the existing pattern, doesn't add a new one):** when a test docstring or comment makes a claim about *why* a code path is deterministic/safe/harmless ("falls through to defaults," "this can't happen," "always returns X"), that claim is exactly as fragile as a production-code comment making the same kind of claim, and deserves the same treatment: verify against a real captured response, not against what the surrounding code *appears* to imply. Writing "confirmed empirically" in a docstring commits you to actually having run the confirmation — grep your own draft for confidence language before treating it as done, and if the confirmation hasn't been run yet, run it before the docstring ships, not after.

**Secondary note — coverage delta was reported honestly, not oversold.** The scoped `pytest --cov-branch` numbers for `app.engines`/`app.services.standardization`/`app.models.enums` were numerically identical before and after this phase (same Stmts/Miss/Branch/BrPart/Cover per file) — the new tests pin values on already-line-covered code paths (in-range `calculate()` inputs, the already-exercised shared `_clamp_to_constraints` helper) rather than opening up previously-dead branches. The task's actual goal was behavior-locking for the refactor, not raw coverage-number growth, and the two are not the same thing; reporting "no coverage change" plainly was the correct call rather than reframing it as a win it wasn't.

---

### 2026-08-22 — Engine relocation (app/engines/, app/models/execution/, 4 enums → predictive/): two deliberate scope decisions, and a mypy verification bug caught by re-checking rather than re-trusting

**Context:** Executed the engine relocation the prior recon confirmed was clean: `app/engines/*` and `app/models/execution/*` (8 files) moved verbatim into a new top-level `predictive/` library; `app/models/enums.py` split into the 4 engine-owned enums (`ModelType`, `ComBaseOrganism`, `Factor4Type`, `EngineType`, now `predictive/models/enums.py`) and the 5 orchestration-only enums (staying in `app/models/enums.py`). Every import site across `app/`, `tests/`, `scripts/`, and `benchmarks/` was repointed; the two deferred/circular-import guards (`app/main.py`'s lifespan, `standardization_service.py::_get_engine_registry`) were preserved, just repointed. Pure relocation — every moved file diffs against its original only on import lines, verified with `diff --strip-trailing-cr` per file before deleting the originals.

**Decision 1 — `ComBaseOrganism.from_text()` / `all_matches_in_text()` travel with the enum, as-is.** These free-text alias-matching methods are called only from orchestration code (`orchestrator.py`, `grounding_service.py`), never from the engine itself — only `from_string()` (exact alias lookup, used by the engine's own CSV-row registration) is engine-internal. Flagged in the recon as a design question, not a blocker: the methods are pure stdlib (no `rapidfuzz`, no `app.*` import), so they don't resist the move technically. Decided to keep them on the enum rather than split them out to an app-side wrapper, on the reasoning that the organism vocabulary and its aliases are a property of the enum itself (a future second consumer of `predictive/` — referred to as "RIM" — will want the same alias resolution, not a reimplementation), and splitting them out would be exactly the kind of premature abstraction `coding_guidelines.md` warns against for a problem that doesn't exist yet. Recorded here, not silently carried over.

**Decision 2 — `StandardizationService` keeps importing `ComBaseModelConstraints` directly, only repointed.** This is the deepest coupling surfaced by the recon: `standardization_service.py` uses the engine's own `ComBaseModelConstraints` pydantic model (with its `clamp_temperature`/`is_ph_valid`/etc. methods) as its own working type for 6 internal method signatures — not just an enum, a full engine-owned type with behavior. The clamping *logic* itself was explicitly out of scope for this move (non-goal: "No clamping decouple (repoint only)"); the import now points at `predictive.engines.combase.models.ComBaseModelConstraints` instead of `app.engines.combase.models.ComBaseModelConstraints`, with zero change to what `standardization_service.py` does with it. The decouple (should orchestration speak in terms of a narrower, orchestration-owned clamping interface instead of the engine's own constraint object?) is deferred as a named next phase, not resolved here.

**What surfaced — a self-correction on a claim from the immediately preceding phase.** The previous phase's mypy report claimed "`mypy app` baseline errors contain ZERO errors from the files that are now moving to predictive/" — checked at the time, but wrong. Re-verifying during this phase (to confirm the mypy-baseline acceptance criterion), a case-sensitive-but-slash-direction-wrong grep (`grep -c "engines\\\\\|execution\\\\"` with a malformed backslash escape in the alternation) silently matched zero lines against Windows-backslash mypy output, producing a false "clean" result. Caught only because this phase re-ran the check from scratch with a `git worktree` at the pre-move commit and a case-insensitive grep, which found 16 real errors (across `app/engines/base.py`, `app/engines/combase/engine.py`, `app/engines/combase/models.py`) that had been part of the 151-error baseline all along. The corrected, load-bearing verification: a line-number-normalized diff (`sed -E 's/:[0-9]+:[0-9]+:/:LN:/'`, then sort, then `diff`) between the pre-move worktree's `mypy app` output and the post-move working tree's `mypy app` output showed **zero net-new errors** — the same 16 diagnostics simply now report under `predictive\` paths instead of `app\engines\` paths (mypy follows the import from `standardization_service.py` and still reports on it, even though `predictive/` isn't itself a command-line target), and all 135 other errors are byte-identical. The relocation is mypy-clean in the way that actually matters (no new errors), but the *previous* phase's specific claim about the baseline's composition was false and is corrected here.

**Heuristic (same family as the "unverified docstring claim" entry immediately above, one level up):** a verification claim about a *tool's output* (mypy, ruff, coverage) is exactly as fragile as a claim about code behavior, and for the identical reason — it's a grep or a diff written once, trusted afterward without being re-run. The specific failure mode here (a malformed shell escape silently matching nothing instead of erroring) is worth naming: `grep -c PATTERN` returning `0` is indistinguishable, from the caller's perspective, between "genuinely zero matches" and "the pattern never had a chance to match anything" — a bad regex and a true negative look identical. When a grep-based verification is the sole evidence for a claim that will be repeated in a report (here: "the baseline contains zero errors from X"), sanity-check the grep itself against a line you already know should match, or switch to a construction (line-number-normalized diff, in this case) that fails loudly rather than silently under-counting.

**Verification technique worth keeping: `git worktree` for "what did this look like before I touched it" comparisons.** Rather than `git stash`/`git show > tempfile` reconstruction (error-prone for multi-file comparisons, as the enum-split verification in the previous phase already showed), `git worktree add --detach <path> HEAD` gives a full, independently-runnable checkout of the pre-change tree in one command, usable for running the *actual* tool (`mypy app`, not a hand-assembled subset of files) against the *actual* pre-change layout, then `git worktree remove --force <path>` to clean up. This is the more reliable pattern whenever "run the same command against the old code" is the actual question, not "diff two file contents."

---

### 2026-08-22 — Clamping decouple: "the engine reports facts, PTM decides policy" as a realized design boundary

**Context:** Follow-on to the engine relocation. `StandardizationService` imported `ComBaseModelConstraints` (an engine type) directly and called its four `clamp_*()` methods to get a clamped value. Recon (same-day, prior turn) established the fact/action split precisely: `ComBaseModelConstraints`'s bound fields and `is_*_valid()` predicates are facts (no mutation, no policy); its `clamp_*()` methods are the only actions, and every one of them is exactly `max(min_field, min(value, max_field))` — a one-line arithmetic reproduction of information already sitting on the object as plain fields. Two commits realized the design: remove the engine's own dead internal-clamp path first (proving deadness before deleting), then move the trivial arithmetic into PTM and delete the four action methods entirely.

**What went well — proving dead code dead before deleting it, not after.** `ComBaseCalculator.calculate()`'s `clamp_to_range` parameter gated four internal clamp branches that the original relocation recon had already flagged as uncovered by tests. Before removing anything, grepped every `calculate()` call site across `app/`, `predictive/`, `tests/`, `scripts/`, `benchmarks/`: `clamp_to_range=True` appeared exactly once in the whole repo, in one unit test; the sole production caller (`engine.py`) hardcoded `False` as a literal, not a variable — meaning no config, no input, no future code path could make it `True` without an explicit code change. This is the strongest kind of dead-code proof available short of a formal reachability analysis: not "no test exercises this" (which the coverage report already said) but "no caller anywhere passes the value that would exercise it, and the one caller that could is a hardcoded literal." Removed the parameter, the four branches, and the one test that exercised them; renamed the adjacent surviving test since "no clamping when disabled" stopped meaning anything once there was no "enabled" alternative to contrast with.

**Second confirmation, independent of the first: output-identity via zero test diff, not just "tests still pass."** After moving the clamp arithmetic into `StandardizationService`, the strongest evidence that PTM's *output* was unchanged wasn't "the full suite is green" (a weaker claim — a coincidentally-compensating bug could still pass a loose test) but that `tests/unit/test_model_type_aware_bias.py::TestRangeClamping`, `tests/unit/test_factor4_standardization.py`, and `tests/integration/test_full_pipeline.py` — the tests that assert *exact* clamped values, `RangeClamp` records, and warning strings end-to-end — showed **zero diff**. A test suite passing after a refactor proves the refactor didn't break anything the tests check; a test *file* showing zero diff while its target code changed proves the specific values those assertions pin didn't move at all. The response-snapshot baseline (four full-response scenarios, byte-identical before and after) was the same proof at a coarser grain.

**What was checked and found NOT to be a problem, worth recording as a negative result:** Step 3 asked to fix "EngineType's placement" only if the recon showed a concrete defect. Re-investigated from scratch (the referenced recon finding didn't actually exist in this project's history under close inspection) — grepped every consumer of `EngineType` repo-wide and found it used exclusively inside `predictive/`'s own execution-result schema (`BaseExecutionResult.engine_type`, `ComBaseModelResult.engine_type`, etc.) and by tests constructing those objects; zero orchestration production code (`app/core/orchestrator.py`, `app/api/`, `app/services/`) imports or references it at all — the one place it reads `.value` off an already-constructed object needs no import. No wrong-module symptom, no import inconsistency, no awkward consumer split. Did nothing, reported it as a note. **Heuristic:** when a task names a "the recon flagged X" defect and the actual investigation can't reproduce the claimed finding, re-derive the answer from the current code rather than searching harder for what might have been meant — "no defect found" is itself the correct, reportable answer, and inventing a defect to have something to fix would be exactly the "restructure for tidiness" the task explicitly warned against.

**Named pattern (ties to the relocation phase's `ComBaseOrganism.from_text()` decision, same session-arc): a type's "home" is decided by who needs the behavior, not by who currently imports it.** The clamp arithmetic moved to PTM not because the engine was doing something wrong, but because *deciding what to do about an out-of-range value* (clamp vs. reject vs. fail closed, and which direction) is a policy choice a second engine consumer might make differently — exactly the shape of thing `coding_guidelines.md`'s "simplicity first" would call premature to generalize *inside* the engine. Keeping the fact (bounds, validity) on the engine and moving the decision to the caller is the same "shared vocabulary vs. shared policy" distinction as the earlier `ComBaseOrganism` methods decision, applied in the opposite direction: there, free-text matching stayed on the engine because it was vocabulary, not policy; here, clamping left the engine because it was policy, not vocabulary.

---

### 2026-08-22 — Baranyi primary growth model: mixed-basis-per-field as a port hazard, and a monotonicity bug caught by the round-trip test it was supposed to protect against

**Context:** Ported the Baranyi-Roberts primary growth model (`predictive/primary/baranyi.py`) from a validated frontend TypeScript implementation, plus a new analytic inverse (time-to-target) the frontend doesn't have. Additive: no change to `ComBaseCalculator`, no pipeline wiring, no `app/` consumer.

**The units hazard, stated precisely because "be careful with units" is useless advice on its own.** The five scenario inputs are not uniformly log10 or ln — `initial_log_cfu` is log10 but the two fields sitting right next to it in every function signature, `y_max` and `mu_max`, are *already* ln. A loop that says "convert every population/rate field to a common basis" is the natural-looking refactor and is exactly wrong: it multiplies `y_max` by `ln(10)` a second time (19.6 → 45.1, an asymptote 25 orders of magnitude too high in real CFU terms) while `initial_log_cfu` needed that exact multiplication. The fix that actually prevents this class of bug isn't a code comment, it's the type signature: `BaranyiParams` names the fields `y_max_ln`/`mu_max_ln` (basis in the name itself, visible at every call site, no docstring required to know not to touch them) versus `initial_log_cfu`/`target_log_increase` (log10, matching the log10 fields this codebase already uses elsewhere — `ComBaseExecutionResult.initial_log_cfu`). There is no generic "list of log values" parameter anywhere in the module for a uniform conversion to have somewhere to be written. Verified the claim empirically, not just by design: `exp(19.6) ≈ 3.3e8` CFU/g (Listeria's real CSV `ymax`) is a plausible bacterial carrying capacity; `10^19.6` CFU/g (what it would mean if `ymax` were log10) is not — independent confirmation the CSV column really is ln-basis, from the data itself, not just from what the frontend spec claims.

**A genuine bug in my own first-draft inverse, caught by the round-trip test it exists to validate — not found by re-reading the math.** The initial `time_to_target()` had an "already met" check that fired whenever `target_log_increase <= 0`, unconditionally. That's correct for a growing scenario (`mu_max_ln > 0`) but backwards for a declining one (`mu_max_ln < 0`, thermal inactivation/survival): asking "when has this dropped by 5 log" produces a *negative* `target_log_increase` by construction (population decreasing = target below the start), and the buggy check treated every one of those as "already met at t=0" — silently returning zero time for a real, computable inactivation duration. The round-trip test (`population_at(T) → target → time_to_target(target)` should recover `T`) caught this immediately: the inactivation branch returned `ALREADY_MET` for every nonzero `t_true`, not the expected `REACHED` with the right time. The fix: "already met" is about *direction*, not sign — `(mu_ln >= 0 and target_ln <= 0) or (mu_ln < 0 and target_ln >= 0)`, i.e., the target is trivially satisfied only when it sits on the side of the starting population the curve is *not* moving toward. This is the same class of bug the growth-scenario test suite alone would never have caught (every growth test has `mu_max_ln > 0` by construction) — the round-trip test's value wasn't "prove the algebra is right," it was "exercise a scenario shape (declining direction) the example-reproduction tests structurally couldn't reach."

**A related, more subtle correction during test-writing: for a monotonic, *unbounded* curve, "wrong direction" is `ALREADY_MET`, never `UNREACHABLE`.** First draft of the test suite included a case expecting `UNREACHABLE` for "asking to increase while the model declines, no asymptote" (`is_growth=False`, i.e. `m_curv=0`, no `y_max` bound). Running it revealed the code returned `ALREADY_MET` instead — and on working through *why*, the code was right and the test's premise was wrong: `time_to_target`'s outcomes are threshold-crossing questions ("has population reached *at least*/*at most* this level"), not exact-equality questions, and for a monotonic function without a bound, a target on the side already passed is definitionally satisfied at `t=0` — there's no way to "fail to reach" a threshold you started past. `UNREACHABLE` for the no-asymptote case can only happen through the genuinely degenerate path (`mu_max_ln == 0`, a flat line). The `m_curv=10` case *does* have a real `UNREACHABLE` (past `y_max`, which the curve approaches but never crosses in finite time) precisely because the asymptote is a bound the monotonic curve can't get past, unlike the unbounded case. Named because it's a reusable check: before writing an "unreachable" test for a monotonic system, ask whether the thing bounding reachability is an actual mathematical bound (asymptote) or just "the wrong direction" — the latter is never unreachable for a monotonic curve, only already-behind-you.

**The inverse derivation, recorded here since the code comments reference this entry.** Baranyi's adjustment function has the closed form `A(t) = t - λ + (1/μ)·ln[1 + e^{-μt}(e^{h0}-1)]` (λ=h0/μ; this is algebraically identical to the reference's `lag`/`tpl` intermediate-variable formulation, verified by direct substitution). Solving for `t` given a target `A*` — substitute `s=μt`, isolate the exponential, and it falls out to `t* = (1/μ)·ln[e^{h0}(e^{μA*}-1) + 1]`, no iteration. The asymptote-corrected population equation (`m_curv=10`) reduces to a single linear equation in `z = e^{(μ/cm)·A}` (`cm=(y_max-y0)/m_curv`) via the same kind of substitution: `z = W·G/(1-W·F)` where `W=e^{(target-y0)/cm}`, `F=e^{-m_curv}`, `G=1-F` — also closed form, also no iteration. Both were verified two ways before being trusted: algebraically (forward substitution back into the original equation) and numerically (the round-trip test, to 1e-13 *within the parameter envelope the test suite exercised* — durations up to 10h growth / 0.5h inactivation, `mu_max_ln` fixed at two representative values) — the algebra alone would not have caught the direction bug above; only running it did. **That scoping caveat matters and was itself corrected after code review — see below**: "verified to 1e-13" had originally been written without qualification, reading as a general robustness claim the tested envelope didn't actually support.

**Code review found what the test suite's own envelope couldn't: a real numerical-stability defect, by fuzzing outside the range the hand-picked test cases covered.** The algebra above is exact, but the *first implementation* of the `A*→t*` step computed `math.exp(h0) * (math.exp(mu_abs*a_star) - 1) + 1` directly. For realistic-not-adversarial inputs — a moderate growth rate over a 12-30 hour temperature-abuse duration approaching `y_max`, or a fast thermal-inactivation rate with `h0>0` and a large-but-real log-reduction target — `mu_abs*a_star` routinely exceeds float64's `exp()` overflow threshold (~709.78). Two distinct silent failure modes, not one: (1) `math.exp()` itself raises `OverflowError` past that threshold — catchable; but (2) a *plain float multiplication* immediately after (`math.exp(h0) * huge_but_still_finite_value`) can overflow to `inf` **without raising anything at all**, and `math.log(inf)` again returns `inf` cleanly rather than erroring — so a naive `try/except OverflowError` around the whole expression closes failure mode (1) while leaving (2) completely open, silently returning `BaranyiInverseResult(REACHED, hours=inf)`. A second, independent defect in the same code-review pass: the `is_growth=True` branch's reachability check computed `(target_y_ln - y_max_ln) * cm >= 0` as one expression, and the `z = w*g_const/(1-w*f_const)` formula as a *separately-rounded* expression of the same underlying near-singularity (`target → y_max`) — the two could disagree in sign right at the boundary (float64 rounding noise flips which side of zero a tiny quantity lands on), so the "reachable" guard could pass while the `z` computation's denominator was actually negative, sending `z` massively negative and `math.log(z)` into a `ValueError` domain crash. Both defects were invisible to every test written so far (worked examples, round-trip at modest durations, hand-picked already-met/unreachable cases) because none of them happened to probe near-saturation or large-magnitude targets — exactly the gap the "scoping caveat" note above was written to flag.

**The fix, and why it generalizes rather than just patching the two reported inputs.** (1) The reachability guard for the `is_growth=True` branch now derives from the *same* quantity (`denom = 1 - w*f_const`, computed once) the `z` formula uses, checked against a small epsilon (`1e-9`, far above float64 rounding noise) rather than a second, independently-rounded expression — this doesn't just fix the one reported input, it removes the entire class of "two views of one singularity disagreeing" by construction. (2) The `A*→t*` inversion was rewritten in log-sum-exp form — `x = h0 + |mu|*A`; `ln(inner) = x + log1p(exp(-x) - exp(h0-x))` — which never exponentiates a *positive* argument at all, only negative ones (safe underflow to 0, never overflow), mirroring exactly how `_dmy_ln` itself avoids the identical failure mode in the *forward* direction (it works with `exp(-h0*tpl)` and `exp(exponent)`, both negative-argument, never `exp(+large)`). This was the more important fix: fuzz-testing 100,000+ random `(y0, y_max, mu, h0, t)` combinations after the fix found zero crashes and zero wrong answers, whereas a narrower "catch the exception and return UNREACHABLE" patch would have silently mis-reported a fair number of those as unreachable when a real, finite, correct time exists (verified directly: the exception-only patch reported UNREACHABLE for a target requiring a perfectly ordinary ~5-hour inactivation, because the *unstable arithmetic*, not the model, was what failed). `math.isfinite()` checks remain as a final defensive layer (not the primary fix) in case a future edit reintroduces a silent-inf path elsewhere in the same function.

**Reused technique, worth naming again — but note where it stopped being enough on its own.** Scratch-verify math in an isolated script before writing it into the production module or its tests (same discipline as the mypy-baseline and clamping-inverse derivations in the two immediately preceding phases). This caught the direction bug and the wrong `UNREACHABLE` test premise. It did **not** catch the numerical-stability defects above, because the scratch verification and the hand-written test cases both used the same handful of "reasonable-looking" parameter values — hand-picked examples share the author's blind spots by construction. What did catch it: an independent code-review pass that generated adversarial/edge-magnitude inputs specifically to probe outside the envelope the implementer's own tests covered, rather than re-reading the same code and the same examples. The standing heuristic this adds: hand-picked verification (however careful) proves the *algebra* is right; only search/fuzzing outside the examples' own parameter range can find where the *floating-point arithmetic* diverges from the algebra — the two are different failure classes and neither test strategy substitutes for the other.

**A third finding from the second code-review pass — and a reviewer claim that didn't survive independent verification.** The review flagged a large-`h0` crash: `h0 >= ~37.5` combined with `a_star` near zero raised `ValueError` from `math.log()`, because `math.expm1(-h0)` underflows to exactly `-1.0` past that threshold and the surrounding `log1p(term_diff)` call received a `term_diff` that, combined with the other term, drove its argument to exactly `-1.0` or below (`log1p`'s domain floor). The review also claimed a related but distinct "silently imprecise, not crashing" band for `h0` roughly 15-37, and suggested the `expm1`-based reformulation would improve precision there too. **Verifying that second claim independently — not just re-applying the suggested fix — found it does not hold.** A systematic sweep comparing the pre-fix formula, the `expm1`-reformulated one, and a bisection oracle across the 15-37 band produced *byte-identical* errors between the two formulas in every case tested: the reformulation is not measurably more precise there. Chasing why led to the actual root cause: `population_at()` — the forward function, already verified against the frontend examples in Part 1 — itself cannot resolve any measurable growth at these `(h0, duration)` combinations. Confirmed directly: `population_at()` returns exactly `y0` (not a small-but-nonzero increment) for `h0=33` at durations up to 2 hours, because a 33-hour lag phase genuinely dwarfs a 2-hour observation window at float64 precision — there is no growth signal left to invert, in either direction. This is a pre-existing precision floor shared identically with the forward path, not a defect the inverse introduced or can fix by reformulating its own algebra; claiming to have "fixed" it would have been dishonest. The fix actually shipped: (1) the `expm1`-based reformulation anyway, since it is mathematically cleaner (one stable primitive instead of subtracting two independently-rounded `exp()` calls) and provably not worse, plus (2) an unconditional `term_diff <= -1.0 → UNREACHABLE` guard closing the genuine crash wall — verified by direct reproduction of the reviewer's `h0=40.0` case (now returns a clean `UNREACHABLE`, no exception) and a sweep across `h0 ∈ {37.5, 37.6, 50, 100, 300}`.

**A fourth, independently-discovered crash, found only by fuzzing wider than either the reviewer's report or my own targeted reproduction cases.** Re-fuzzing 200,000 random parameter combinations after applying the `h0` fix above (deliberately widened past every range exercised so far — `h0` up to 300, `mu` up to ±8, targets up to ±80 log10) surfaced a fifth failure mode the second review round never reported: `math.exp((target_y_ln - y0_ln) / cm)` in the `is_growth=True` branch's `w` computation can overflow *directly*, before the `denom <= 0` "past the asymptote" guard downstream ever runs, whenever the target sits far beyond reach relative to a narrow asymptote-correction band (`cm` small, i.e. `y_max` close to `y0`). This is independent of `h0` entirely — reproduced with `h0=35.3` but the crash mechanism has nothing to do with the lag term. Fixed the same way as the log-sum-exp technique above: compute `shifted = exponent - m_curv` (`= ln(w·f_const)`) from plain arithmetic, without exponentiating, and short-circuit to `UNREACHABLE` when `shifted >= 0` (algebraically identical to "`denom <= 0`," decided without the exponentiation that would overflow to reach it) — once `shifted < 0`, `exponent = shifted + m_curv` is bounded above by `m_curv` (10), so the subsequent `math.exp()` calls cannot overflow. Verified by exact reproduction of the crashing input (now returns `UNREACHABLE`) and a re-run of the 200,000-combination fuzz sweep at the widened range (zero crashes, zero invalid `REACHED` results). **The lesson this adds on top of the previous entry's "fuzz outside the hand-picked range":** a single fuzz pass at one parameter range is not a completeness proof — each round of fuzzing here found a *different* failure mode than the round before it, because each fix's own reasoning implicitly narrowed what the next fuzz pass happened to probe. The generalizable defense turned out to be structural, not exhaustive: every `math.exp()` call in this function's hot path is now preceded by an argument-boundedness argument recorded in a comment (why the exponent can't exceed some concrete constant), rather than relying on a `try/except OverflowError` or on having fuzzed enough cases — a wide fuzz sweep is confirmation of that structural argument, not a substitute for it.

**A fifth crash, found by a third code-review pass's own independent fuzzing after the fourth fix above — the pattern held one more time.** Despite that fourth fix closing every `math.exp()` overflow site with a boundedness argument, `cm = (y_max_ln - y0_ln) / m_curv` followed immediately by `exponent = (target_y_ln - y0_ln) / cm` could raise `ZeroDivisionError`: `y_max_ln - y0_ln` can underflow to exactly `0.0` when the two are within about a ULP of each other at the subnormal-float extreme (`y_max_ln` on the order of `1e-323`), a case the top-of-function `(y_max_ln-y0_ln)*mu_ln <= 0` guard doesn't catch because it only inspects a *sign*, not whether the magnitude is about to underflow on division. This is one arithmetic step *upstream* of every guard added by the fourth fix — a reminder that "every `exp()` call is now bounded" is not the same claim as "every division is now guarded," and a structural argument about one class of operation (exponentiation) doesn't transfer for free to a different class (division) sitting immediately next to it. The reviewer's own crash-freedom fuzzing (50,000 iterations, gaps spanning `1e-15` to `1e-300`) and my own follow-up sweep (deliberately re-run with gaps drawn from `{1e-300, 5e-324 (the smallest positive subnormal double), 1e-320, 0.0}` specifically, rather than a general random range that would round such tiny gaps away by construction before ever reaching this line) both needed to *target* the subnormal boundary directly to reproduce it — a uniform-random fuzz sweep over a wide range, even a very wide one, under-samples an interval this narrow (roughly the last 1-2 ULPs before zero, out of a range spanning hundreds of orders of magnitude) essentially by definition. Fixed with the same shape as the fourth fix: `if cm == 0.0: return UNREACHABLE` immediately after computing `cm`, before it is ever used as a divisor — an asymptote gap this tight is, for float64's purposes, indistinguishable from "no room left to grow," the same "no signal left to invert" conclusion the h0 precision-floor paragraph above reaches by a different route. **The added heuristic:** when fixing a numerical-stability class of bug (here: "guard every operation that could blow up on an extreme input"), enumerate the operation *types* at risk (exponentiation, division, subtraction-then-log, ...) explicitly rather than fixing site-by-site as fuzzing happens to find them — division sitting two lines before an already-fixed exponentiation is an easy site to miss precisely because attention was on the just-fixed exponentiation next to it.
