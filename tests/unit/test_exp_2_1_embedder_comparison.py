...
class TestLoadCorpus:
    ...
    def test_positive_entries_have_food_names(self, corpus):
        ...
        for q in positives:
            ...
            assert (
                len(q["acceptable_food_names"]) >= 1
            ), f"{q['id']} has empty acceptable_food_names but tier={q['tier']}"

    def test_negative_controls_have_empty_acceptable_list(self, corpus):
        negatives = [q for q in corpus if q["tier"] == "hard"]
        for q in negatives:
            assert (
                q["acceptable_food_names"] == []
            ), f"{q['id']} hard-tier entry should have empty acceptable_food_names"
    ...
    def test_production_tier_values(self, corpus):
        ...
        for q in corpus:
            assert (
                q["production_tier"] in valid_tiers
            ), f"{q['id']} has unknown production_tier: {q['production_tier']}"

    def test_field_values(self, corpus):
        valid_fields = {"ph_aw_combined", "ph", "water_activity", "pathogen"}
        for q in corpus:
            assert (
                q["field"] in valid_fields
            ), f"{q['id']} has unknown field: {q['field']}"
...
class TestGetThreshold:
    def test_tier_1(self):
        ...
        assert _get_threshold("tier_1") == pytest.approx(
            settings.food_properties_confidence
        )

    def test_tier_2(self):
        from app.config import settings

        assert _get_threshold("tier_2") == pytest.approx(
    ...
    def test_pathogen_stage_1(self):
        ...
        assert _get_threshold("pathogen_stage_1") == pytest.approx(
            settings.pathogen_hazards_confidence
        )
...
class TestComputeEmbedderSummary:
    ...
    def test_mean_expected_score_excludes_nulls(self):
        ...
        assert summary["mean_expected_score"] == pytest.approx(
            (0.80 + 0.75 + 0.58 + 0.65 + 0.78) / 5
        )
...
class TestAllQueriesErroredPipeline:
    ...
    def test_save_results_writes_valid_json(self, tmp_path, monkeypatch):
        ...
        query_results = run_embedder_queries(
            self._AlwaysErrorCollection(), corpus, top_k=5
        )
    ...
    def test_print_summary_does_not_crash_on_none_accuracy(self):
        ...
        embedder_results = [
            self._make_embedder_result(
                run_embedder_queries(
                    self._AlwaysErrorCollection(), load_corpus()[:2], top_k=5
                )
            )
        ]
...
class TestComputeRecommendation:
    ...
    def test_hard_regression_blocks_switch_bug1_regression(self):
        ...
        candidate = _make_embedder(
            is_baseline=False, top1=0.87, hard=5 / 6, name="mpnet"
        )
